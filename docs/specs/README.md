...
> Authored via the `authoring-prds/specs/llds` skills. Altitude discipline and sync mechanics: see [`../process.md`](../process.md) and each skill's `references/document-relationships.md`.
...
The join-key table that turns "are these in sync?" into a coverage check. A `PRD-G#` with no `SPEC-R#` is a gap; a `SPEC-R#` with no `PRD-G#` is scope creep; a `SPEC-R#` with no `LLD-C#`/test is unimplemented. Run `python ../../.claude/skills/authoring-prds/scripts/trace_check.py <prd> <spec> <lld>` to check mechanically.
