...
**LLD-C7 functions:** pure implementations keyed by name — `required(v)→{valid:!isEmpty(v)}`, `email`, `regex(pattern)`, `formatString(tmpl,...args)` (interpolates resolved bindings). The renderer's function evaluator (renderer LLD-C10) looks them up from the bound catalog's `functions`. Unknown name → `FUNCTION` (renderer surfaces).

**LLD-C8 theming:** `applySurfaceProperties(host, props, catalog)` validates `props` against `catalog.surfaceProperties` then sets CSS custom properties (`--ui-*` / `--c-{family}-{role}`) on the surface host — **pure CSS-variable repoint, no JS restyle** (SPEC-R5 AC2), consistent with the dimensional token system. v0.9.x `theme` is normalized to `surfaceProperties` by the version adapter (renderer LLD-C2).
