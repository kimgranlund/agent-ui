...
| **LLD-C10** | Client-side function evaluator + `${…}` interpolator | SPEC-R10 | `functions.ts` · `interpolate.ts` (ADR-0027 path arm) |
...
- **Catalog functions (LLD-C7):** the pure `required(args)→{valid,message?}` / `email` / `regex` trio, looked up from the bound catalog's `functions` registry. **String composition is the DynamicString `${…}` interpolation feature, NOT a `formatString` function** (ADR-0026 / catalog SPEC-R5) — its **path arm is now delivered** (see the interpolator below, ADR-0027); the `${fn(arg:val)}` **function-expression arm** and **`checks`-surfacing** remain follow-ups (`checks`: a component `checks:[{call,args,message}]` array runs each entry through this evaluator, but the field-error display channel + a `VALIDATION_FAILED` code are a dependent follow-up; SPEC-R10 AC1's *surfacing* rides on that, this LLD delivers the evaluation).

**`${…}` DynamicString interpolation (`interpolate.ts`, ADR-0027 — path arm).** A literal **string** value is no longer always opaque: a string with an **unescaped** `${…}` is an interpolation template. `resolveValue`'s literal arm becomes `isInterpolated(s) ? interpolate(s, surface, itemScope, resolve) : s` — a non-`${` string is byte-identical. The interpolator is a small string parser layered on the **existing** per-path resolver, orthogonal to the `{call}` value kind:
- **Scan (escape-aware).** Split the template into ordered literal runs and `${…}` expressions: `\${` emits a literal `${` (backslash consumed); an unescaped `${` opens an expression closing at its matching `}` (brace-depth tracked so a future nested `${…}` stays parse-stable). This scanner is the **forward-compat seam** — unchanged when the function-expression arm lands; only the classifier grows.
- **Classify.** A body **without** `(` is a **JSON-Pointer path** (this wave); a body **with** `(` is the deferred **function-expression** form.
- **Resolve (reuse LLD-C5).** A path expression resolves through `resolve({path: expr}, surface, itemScope)` — the **same per-path memo**; a **relative** path uses `scopedPointer(expr, itemScope)` (ADR-0024), identical to the `{path}`/`@index` read rewrite. No new resolution machinery.
- **Coerce + concat (spec-exact).** number/boolean → `String(v)`; **null/undefined → `""`**; object/array → `JSON.stringify(v)`; string → itself. Segments join in source order.
- **Reactive for free.** Running inside the bound-prop `effect` (LLD-C7), each `${/path}` reads the per-path computed, so the effect depends on exactly the embedded paths — a template re-resolves when **any** changes and stays asleep on an unrelated write (SPEC-N2 reused; this is the v1.0 "Text label updates as the TextField types" behavior).
- **Malformed/deferred → render literally, NO error** (ADR-0027 design decision; the §9 table adds no interpolation code): a deferred `${fn()}` segment and an unterminated `${` are emitted **verbatim** into the output string — no `FUNCTION`, no new code. The function-expression follow-up upgrades the *same* segment to resolve (additive, nothing retracted). Conformance is a **no-op**: a template is a `string` literal, so `conformance.ts matchesType` accepts it via the ordinary string-type leg (no `{path}`/`{call}` branch, no false `CATALOG`).
