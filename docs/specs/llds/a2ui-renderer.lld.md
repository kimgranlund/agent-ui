...
- **Lookup + `callableFrom` gate.** `handleCallFunction` looks `call` up across the renderer's **registered catalogs** (`CatalogRegistry`). It **rejects** — `emitError(INVALID_FUNCTION_CALL, functionCallId)` (the ADR-0031 wire arm, now reachable; `surfaceId` excluded) — when the function is **unregistered** OR its `FunctionDef.callableFrom === 'clientOnly'` (default). It **invokes** a `remoteOnly`/`clientOrRemote` function.
