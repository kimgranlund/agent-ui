...
> | **Supersedes / Superseded by** | **Supersedes ADR-0033 — the font/icon `pow`-derivation MECHANISM only.** ADR-0033's **height-stays-linear** leg stood at this ADR's time *(since superseded by **ADR-0038** (ADR-0037 folded in) — the control ramp becomes Kim's explicit `(scale × size) → §1-row` lookup; this ADR's explicit-table MECHANISM stays, but its non-default **font/icon VALUES are re-tabled** by ADR-0038 to Kim's rows, and the `× --ui-scale`/snapped-from-multiplier framing goes)*, and its **sublinear / §1-faithful OUTCOME** stands (font grows slower than height, landing on the §1 ramp). What reverses is the *realization*: `--ui-font = base × pow(scale, 0.45)` (a continuous approximation that yields decimals like 20.6) → an **explicit per-tier table of the §1 SET integers** (12·13·14·16·18·20). Mark ADR-0033 *"font/icon pow mechanism superseded by ADR-0035; height-linear leg + sublinear outcome stand."* **Relates ADR-0032/0025.** |
...
- **ADR-0033's font/icon mechanism is superseded; its sublinear outcome stands** — the realization chains converge at **ADR-0038** (Kim's `(scale × size) → §1-row` lookup, no multiplier): font `ADR-0007 (linear) → 0033 (pow) → 0035 (explicit §1 table) → 0038 (Kim's lookup; values re-tabled)`; height `ADR-0007 (linear) → 0037 (snap, folded) → 0038`. The *intent* (§1-faithful integers, height+glyphs consistent) is preserved across the chain; only the *how* changes.
...
- **S3 — `dimensions.test.ts`** (source ripple): the `--ui-font`/`--ui-icon` assertions → the explicit-table values per `[scale]` selector (replacing the `pow` assertions); assert height stays linear *(superseded by ADR-0038 — controls drop `× --ui-scale` for Kim's explicit lookup; that assertion flips there)* + `--ui-type-*` stays linear.
