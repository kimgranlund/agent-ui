...
> | **Repairs** | `docs/plan.md` §8 (the CSS-trio + per-component file set), `docs/goals.md` §G5 DoD (the styling bullet), `docs/process.md` §1 (the naming/structure trip-wire's "exact per-component file set"), the `authoring-components` skill (scaffold step 2 + CSS-trio step 5). *(No edit: the import-layering trip-wire — `controls/` stays the FACE layer.)* |
> | **Supersedes / Superseded by** | *(none)* |

## Context

Ratified G5 design (`docs/plan.md` §8) packages every component as a folder holding a **CSS trio** —
...
every later control (and the `authoring-components` procedure) copies, and Phases 2–4 (demo, `/site`,
...
`authoring-components` (which now scaffolds `{name}.{ts,css,md}`).
...
- **Propagation:** the `authoring-components` scaffold + the (unbuilt) naming/structure + token-hygiene
