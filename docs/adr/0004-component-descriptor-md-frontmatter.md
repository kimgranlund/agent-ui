...
> | **Repairs** | `docs/plan.md` §10 (the `{name}.api.json` API-contract instrument), `docs/process.md` §1 + §4 (the contract↔props trip-wire target + the "api-contract schema" referential row), `docs/goals.md` §G5 DoD (the `button.api.json` bullet), the `authoring-components` skill (descriptor step 6). |
...
- **Negative — YAML frontmatter is less strictly typed than schema-validated JSON** unless enforced; so the
  ...
  swap.
- **Propagation:** `authoring-components` now scaffolds `{name}.{ts,css,md}`; the (unbuilt) contract
