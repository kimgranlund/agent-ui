...
> | **Repairs** | `docs/goals.md` §G2 verdict (the "lazy-upgrade attribute-wins vs property-wins" deferral), `docs/plan.md` §5 (the lazy-property upgrade dance). |
...
every later control and by the `authoring-components` procedure. Even though `variant`/`size`/`disabled`
