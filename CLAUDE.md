...
- `npm run test:browser` — the real-engine gate, FIVE SEQUENTIAL shards (packages:components →
  packages:app → packages:rest → site → visual; GH #41 — never re-monolith it or add a heap bump; a
  single shard nearing the default ceiling splits further — the packages project split 2026-07-19
  (measured red on main pre-diff), and :rest split again the same day when it flipped 134 under load)
