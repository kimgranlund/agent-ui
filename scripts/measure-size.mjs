...
const targets = [
  ...
  // ratchet.
  // RE-BASED 2026-08-21 (GH #1567, svg-charts wave 3 — component-build seat's own measured run): 70 KB ->
  // 70.5 KB (72192 B gz) -- this wave's own mover is the `ui-gauge` mint (ADR-0229 cl.4): measured 71800 B
  // gz on this branch (a +120 B gz overage past the 71680 checkpoint), consistent with the control's own
...
const MARGINAL_OVERRIDES = {
  ...
  'line-chart': [2112, 'gzip measurement-frame drift class (the split/status-stream/otp-field precedent, first crossing here): line-chart source untouched — it carried no override at all before this wave, sitting comfortably under the generic 2048 B gz cap — while GH #1567 (svg-charts wave 3) grew the family bundle with the `ui-gauge` mint, shifting the leave-one-out frame; measured 2057 B gz 2026-08-21, npm ci fresh, ~55 B headroom.'],
  'otp-field': [2496, 'Kim ruling 2026-08-08 (host round): LLD-mandated machinery (total reducer + paste-split + echo channel), reviewer confirmed no remaining fat — measured 2150 B gz 2026-08-08 (code-entry-control.lld.md, GH #490 S2-a); re-bumped 2304 → 2400 for the SAME gzip measurement-frame drift class (the split/status-stream precedent): otp-field source untouched while GH #1566 (svg-charts wave 2) grew the family bundle with ui-line-chart\'s axes state — measured 2399 B gz 2026-08-21, npm ci fresh; re-bumped 2400 → 2496 for the SAME drift class a further time: otp-field source untouched while GH #1567 (svg-charts wave 3) grew the family bundle with ui-gauge — measured 2449 B gz 2026-08-21, npm ci fresh.'],
  'drill': [2304, 'gzip measurement-frame drift class (the split/status-stream/otp-field/line-chart precedent), first crossing here: drill.ts is untouched by GH #1568 (svg-charts wave 4, a catalog/JSON/test-only follow-up with zero `@agent-ui/components` source changes) — it carried no override at all before this wave, sitting comfortably under the generic 2048 B gz cap, but GH #1567 (svg-charts wave 3, already merged) grew the family bundle with the `ui-gauge` mint and shifted the leave-one-out frame; the drift was pre-existing on main and only surfaced when this wave ran the gate — verified via a stash-and-remeasure against origin/main (533587f6), which reproduces the SAME 2275 B gz reading with none of this wave\'s a2ui-only diff applied. Measured 2275 B gz 2026-08-21, npm ci fresh, ~29 B headroom.'],
}
