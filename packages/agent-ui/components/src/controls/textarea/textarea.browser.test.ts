...
describe('ui-textarea -- ADR-0223 two postures (fill default / [inline] hug, both engines)', () => {
  it('bare host offsetWidth ~= container inline size (fill); [inline] host >= its floor and < container (hug)', async () => {
    ...
    wrap.innerHTML = `<ui-textarea label="Notes"></ui-textarea>`
    document.body.append(wrap)
    mounted.push(wrap)
    const host = wrap.querySelector('ui-textarea') as HTMLElement & { updateComplete: Promise<unknown> }
