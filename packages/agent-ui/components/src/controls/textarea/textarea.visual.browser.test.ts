...
// `__baselines__/textarea.visual.browser.test.ts/<name>-chromium-darwin.png`; re-baseline only via
...
const mount = (attrs = ''): { wrap: HTMLElement; host: HTMLElement } => {
  ...
  wrap.innerHTML = `<ui-textarea label="Notes" placeholder="Type here" ${attrs}></ui-textarea>`
  document.body.append(wrap)
  return { wrap, host: wrap.querySelector('ui-textarea') as HTMLElement }
}

describe('ui-textarea -- visual regression (ADR-0223 slice-1 postures: fill default / [inline] hug)', () => {
  it.skipIf(server.browser !== 'chromium')('FILL (default): a bare ui-textarea stretches to its block container', async () => {
    const { wrap, host } = mount()
    await (host as HTMLElement & { updateComplete: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('textarea-fill-default')
    wrap.remove()
  })

  it.skipIf(server.browser !== 'chromium')('[inline] hug: the ui-textarea hugs at its content floor', async () => {
    const { wrap, host } = mount('inline')
    await (host as HTMLElement & { updateComplete: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('textarea-inline-hug')
    wrap.remove()
  })
})
