...
// here — this file proves the STATE MACHINE.
...
describe('UIDrillElement — uncontrolled path resolution + self-mutation', () => {
  ...
  it('drilling forward (a real click on a data-role="drill-trigger" element) self-mutates + shows the next panel', () => {
    const el = makeTree()
    const onChange = vi.fn()
    el.addEventListener('change', onChange)
    el.querySelector('[data-drill-key="settings"]')!.dispatchEvent(new MouseEvent('click', { bubbles: true }))
  ...
  it('drilling into a HIDDEN (inactive) panel\'s trigger does nothing (only the active panel\'s triggers fire)', () => {
    const el = makeTree()
    ...
    appearanceTrigger?.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    expect(el.effectivePath).toEqual(['root'])
    el.remove()
  })

  it('Back pops one level; a no-op at the root', () => {
    const el = makeTree()
    el.querySelector('[data-drill-key="settings"]')!.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    expect(el.effectivePath).toEqual(['root', 'settings'])
    el.parts.back?.click()
    expect(el.effectivePath).toEqual(['root'])
    el.parts.back?.click() // already at root — no-op, no throw
    expect(el.effectivePath).toEqual(['root'])
    el.remove()
  })

  it('Escape is a Back alias', () => {
    const el = makeTree()
    el.querySelector('[data-drill-key="settings"]')!.dispatchEvent(new MouseEvent('click', { bubbles: true }))
  ...
  it('the back button is hidden at the root and visible one level deep, with a resolvable parent-heading label', () => {
    const el = makeTree()
  ...
  it('the heading part mirrors the active panel\'s heading prop', () => {
    const el = makeTree()
...
describe('UIDrillElement — controlled path (ADR-0102 prop-as-source-of-truth, the ui-split.sizes precedent)', () => {
  it('renders the controlled `path` and does NOT self-mutate on a drill-forward commit', () => {
    const el = makeTree()
    el.path = ['root']
    expect(el.effectivePath).toEqual(['root'])
    const onChange = vi.fn()
  ...
  it('a broken/unresolvable controlled path falls back to the last resolvable entry, never throws', () => {
    const el = makeTree()
  ...
  it('an empty controlled path ([]) resolves to the root, same as unset', () => {
    const el = makeTree()
    el.path = []
    expect(el.effectivePath).toEqual(['root'])
    el.remove()
...
describe('UIDrillElement — focus priming', () => {
  it('does NOT steal focus on initial mount, but focuses the heading on a later path change', () => {
    const el = makeTree()
