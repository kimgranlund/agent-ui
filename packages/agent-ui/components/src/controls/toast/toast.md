...
parts:
  ...
  - name: close
    description: The ALWAYS-present icon-only <ui-button data-part="close" variant="ghost">, containing a decorative <ui-icon name="x"> and carrying aria-label="Dismiss" (the icon-only-button idiom — the accessible name has nothing to read from textContent). Activating it closes the toast.
...
connect, and an icon-only `<ui-button data-part="close">` (`<ui-icon name="x">`, `aria-label="Dismiss"`)
