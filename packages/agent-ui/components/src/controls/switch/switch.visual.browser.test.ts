...
// Baselines commit under `__baselines__/switch.visual.browser.test.ts/<name>-chromium-darwin.png`;
...
const mount = (attrs = ''): { wrap: HTMLElement; host: HTMLElement } => {
  ...
  wrap.innerHTML = `<ui-switch ${attrs}>Wi-Fi</ui-switch>`
  document.body.append(wrap)
  return { wrap, host: wrap.querySelector('ui-switch') as HTMLElement }
}

describe('ui-switch -- visual regression (ADR-0223 slice-2 postures: fill default / [inline] hug)', () => {
  it.skipIf(server.browser !== 'chromium')('FILL (default): a bare ui-switch stretches to its block container', async () => {
    const { wrap, host } = mount()
    await (host as HTMLElement & { updateComplete?: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('switch-fill-default')
    wrap.remove()
  })

  it.skipIf(server.browser !== 'chromium')('[inline] hug: the ui-switch hugs its content', async () => {
    const { wrap, host } = mount('inline')
    await (host as HTMLElement & { updateComplete?: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('switch-inline-hug')
    wrap.remove()
  })
})
