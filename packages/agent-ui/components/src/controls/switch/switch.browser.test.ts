...
describe('ui-switch cross-engine browser smoke — track + thumb geometry (ADR-0041)', () => {
  ...
  it('track inline-size ≈ 1.8× the box; the pill is wider than tall (density-invariant)', () => {
    // ADR-0223 (Fill by Default, slice 2): the default host now FILLS the container, so the host-width-
    // equals-track claim lives in the `[inline]` hug state (the pre-wave posture; a bare label-less switch).
    const { sw } = mount('<ui-switch inline></ui-switch>') // md default: box = 16px, track ≈ 28.8px
    const h = trackHeight(sw)
...
describe('ui-switch cross-engine browser smoke — solid off-track (SC 1.4.11, ADR-0059)', () => {
  it('the UNCHECKED track (::before) paints an OPAQUE neutral — --md-sys-color-neutral-track resolved, not the old translucent outline-variant', () => {
    // The off-track was --md-sys-color-neutral-outline-variant (neutral-500 @ 40%), which composited to only 1.51:1
    // light / 1.73:1 dark over the surface — an SC 1.4.11 fail; the near-white thumb didn't carry it either
    // (1.22–1.84:1 in the off state). The fix is the SOLID --md-sys-color-neutral-track. A headless env can't paint the
    ...
    expect(alphaOf(bg), `off-track background must be OPAQUE (solid --md-sys-color-neutral-track), got "${bg}"`).toBe(1)
  })
})
...
describe('ui-switch -- ADR-0223 two postures (fill default / [inline] hug, both engines)', () => {
  it('bare host offsetWidth ~= container inline size (fill); [inline] host hugs below the container', async () => {
    ...
    wrap.innerHTML = `<ui-switch>Wi-Fi</ui-switch>`
    document.body.append(wrap)
    const host = wrap.querySelector('ui-switch') as HTMLElement & { updateComplete?: Promise<unknown> }
