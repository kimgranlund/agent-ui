...
// Baselines commit under `__baselines__/checkbox.visual.browser.test.ts/<name>-chromium-darwin.png`;
...
const mount = (attrs = ''): { wrap: HTMLElement; host: HTMLElement } => {
  ...
  wrap.innerHTML = `<ui-checkbox ${attrs}>Accept the terms</ui-checkbox>`
  document.body.append(wrap)
  return { wrap, host: wrap.querySelector('ui-checkbox') as HTMLElement }
}

describe('ui-checkbox -- visual regression (ADR-0223 slice-2 postures: fill default / [inline] hug)', () => {
  it.skipIf(server.browser !== 'chromium')('FILL (default): a bare ui-checkbox stretches to its block container', async () => {
    const { wrap, host } = mount()
    await (host as HTMLElement & { updateComplete?: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('checkbox-fill-default')
    wrap.remove()
  })

  it.skipIf(server.browser !== 'chromium')('[inline] hug: the ui-checkbox hugs its content', async () => {
    const { wrap, host } = mount('inline')
    await (host as HTMLElement & { updateComplete?: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('checkbox-inline-hug')
    wrap.remove()
  })
})
