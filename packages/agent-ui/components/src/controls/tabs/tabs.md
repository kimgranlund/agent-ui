...
# marginal: ui-tabs measures 1421 B gz (re-measured post the overflow-menu build) at the components-barrel tier — the delta of `npm run size`'s components barrel WITH vs. WITHOUT this control's export, tree-shaken (the tabs compound: ui-tabs + ui-tab + ui-tab-panel, now also composing the shipped `ui-menu` overflow part). Within the per-control ≤ ~2 kB tier budget (plan §10) at THIS tier. NOTE — a DIFFERENT tier is NOT clean: the `@agent-ui/app` curated bundle (super-shell+master-detail+settings+surface-host+conversation+nav-rail, which reaches ui-tabs via its settings screen) grew from 81692 to 82565 B gz, 645 B OVER its 81920 B gz marginal budget — flagged for a coordinator/Kim-ruled remedy (a budget bump, the "table"/"status-stream" override precedent, or a follow-up lazy-load amendment), not silently patched here. Gzip measurement-frame drift (the split.css/toolbar.css precedent) accounts for incidental run-to-run swings on TOP of this real growth.
