...
A default-stamped paddles overlay reserves its own band as **track geometry** (a transparent inline
border sized by `--ui-swiper-paddle-band`, which slide content clips at — GH #1330), so the paddles
never occlude slide content at any scroll position; an author-placed anchor reserves nothing and keeps
whatever layout the author gave it.
...
An opt-in-by-support layer, byte-identical fallback on an engine that lacks it:
