...
describe('ui-select -- ADR-0223 two postures (fill default / [inline] hug, both engines)', () => {
  it('bare host offsetWidth ~= container inline size (fill); [inline] host >= its floor and < container (hug)', async () => {
    ...
    wrap.innerHTML = `<ui-select label="Fruit"><div role="option" value="a">Option A</div></ui-select>`
    document.body.append(wrap)
    mounted.push(wrap)
    const host = wrap.querySelector('ui-select') as HTMLElement & { updateComplete: Promise<unknown> }
