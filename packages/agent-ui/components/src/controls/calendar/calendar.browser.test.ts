...
describe('ui-calendar -- ADR-0223 two postures (fill default / [inline] hug, both engines)', () => {
  it('bare host offsetWidth ~= container inline size (fill); [inline] host hugs below the container', async () => {
    ...
    wrap.innerHTML = `<ui-calendar></ui-calendar>`
    document.body.append(wrap)
    const host = wrap.querySelector('ui-calendar') as HTMLElement & { updateComplete?: Promise<unknown> }
