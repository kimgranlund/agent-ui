...
const mount = (attrs = ''): { wrap: HTMLElement; host: HTMLElement } => {
  ...
  wrap.innerHTML = `<ui-toggle ${attrs}>Bold</ui-toggle>`
  document.body.append(wrap)
  return { wrap, host: wrap.querySelector('ui-toggle') as HTMLElement }
}

describe('ui-toggle -- visual regression (ADR-0223 slice-2 postures: fill default / [inline] hug)', () => {
  it.skipIf(server.browser !== 'chromium')('FILL (default): a bare ui-toggle stretches to its block container', async () => {
    const { wrap, host } = mount()
    await (host as HTMLElement & { updateComplete?: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('toggle-fill-default')
    wrap.remove()
  })

  it.skipIf(server.browser !== 'chromium')('[inline] hug: the ui-toggle hugs its content', async () => {
    const { wrap, host } = mount('inline')
    await (host as HTMLElement & { updateComplete?: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('toggle-inline-hug')
    wrap.remove()
  })
})
