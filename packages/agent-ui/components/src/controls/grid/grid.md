...
`ui-grid` establishes **no `container-type` of its own** (ADR-0100 — a layout primitive is intrinsically
sized, so it can never safely be a query container). This is moot for `ui-grid`'s own responsiveness — the
auto-fit/`minmax` track model above is **track sizing**, not a container query — so the removal leaves its
behavior byte-for-byte unchanged; it only matters for descendants that might themselves want to query, which
now resolve against the nearest externally-sized ancestor boundary instead.
