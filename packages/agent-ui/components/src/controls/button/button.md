...
`icon-only` names the **structure** ("adornment-only, no label"), not the adornment's role — it is
role-agnostic by construction. A **caret-only** button (a lone `data-role="caret"` glyph, no icon, no
label) is already legal today via `<ui-button icon-only aria-label="…">` plus a `data-role="caret"`
adornment: the attribute selects the single-column square, the cell stays icon-sized, and the role
sizes the glyph exactly as it does everywhere else (ADR-0171).
