...
properties:               # IDL beyond attributes-as-API — the imperative streaming contract (ADR-0122 F4)
  ...
  - name: setPendingSource
    description: 'Method — setPendingSource(source: Promise<unknown> | AsyncIterable<unknown> | null) => void (GH #974/ADR-0191, GH #999). Attaches (or, with `null`, clears) the async source this stream is CURRENTLY waiting on — the next incoming chunk from an A2UI streaming surface or the agent Session seam. Wires the `pendingComputed` trait''s query-scoped `pending` signal to the `pending` custom state (see customStates). The consumer''s own responsibility to call this each time it starts or resolves a wait — never inferred from appendEntry/update/finalize/fail, which are already-ARRIVED facts. Composes with the `settled` state rather than being cleared by it: a settled stream can go pending again on a fresh follow-up query without leaving `settled`.'
...
customStates:              # GH #239/ADR-0159 — the disclosure state; the completion invariant still rides the ITEM's own :state(truncated) (timeline-item.md)
  ...
  - pending                # GH #974/ADR-0191 (GH #999) — set via pendingComputed while a consumer-attached setPendingSource() source is in flight, cleared on settle/null; CSS dims the whole strip (--ui-pending-* via this control's own chain). Composes WITH `settled` rather than being cleared by it — a settled stream can go pending again on a fresh follow-up query
