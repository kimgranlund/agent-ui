...
// Baselines commit under `__baselines__/radio.visual.browser.test.ts/<name>-chromium-darwin.png`;
...
const mount = (attrs = ''): { wrap: HTMLElement; host: HTMLElement } => {
  ...
  wrap.innerHTML = `<ui-radio value="a" ${attrs}>Choice A</ui-radio>`
  document.body.append(wrap)
  return { wrap, host: wrap.querySelector('ui-radio') as HTMLElement }
}

describe('ui-radio -- visual regression (ADR-0223 slice-2 postures: fill default / [inline] hug)', () => {
  it.skipIf(server.browser !== 'chromium')('FILL (default): a bare ui-radio stretches to its block container', async () => {
    const { wrap, host } = mount()
    await (host as HTMLElement & { updateComplete?: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('radio-fill-default')
    wrap.remove()
  })

  it.skipIf(server.browser !== 'chromium')('[inline] hug: the ui-radio hugs its content', async () => {
    const { wrap, host } = mount('inline')
    await (host as HTMLElement & { updateComplete?: Promise<unknown> }).updateComplete
    await expect.element(page.elementLocator(wrap)).toMatchScreenshot('radio-inline-hug')
    wrap.remove()
  })
})
