...
```html
<ui-stat label="Revenue" figure="48200" delta="12" caption="vs last month"></ui-stat>
<ui-stat label="Uptime" figure="99.98%"></ui-stat>
<ui-stat variant="ring" label="Storage used" figure="72%" percent="72"></ui-stat>
```
...
`figure` (renamed from `value` — TKT-0069 item 1: `value` is the reserved FACE form value; the A2UI catalog keeps wire
`value`, mapped in its factory) accepts a `string | number`: a finite number formats through the platform default-locale
