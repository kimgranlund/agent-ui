...
parts:                 # the contenteditable editable surface is a control-owned PART, not a slot (ADR-0014 cl.1)
  ...
  - name: clear-button
    description: Inside the trailing-adornment for type=search. A `<button>` (aria-label="Clear"), injected with the Phosphor `x` glyph via `setIcon`, that clears the value and emits input + change. Hidden by CSS when the field is empty (`:has([data-part="editor"][data-empty])`).
  - name: reveal-button
    description: Inside the trailing-adornment for type=password. A `<button>` (aria-pressed="false"/"true") that toggles password masking (-webkit-text-security: disc/none) by adding/removing :state(revealed) and emits a `toggle` event. The glyph itself reflects the state — Phosphor `eye` when masked (click to reveal), `eye-slash` when revealed — swapped via `setIcon` alongside aria-pressed/aria-label.
  - name: step-up
    description: Inside the trailing-adornment for type=number/currency/unit/percent. A square, icon-sized `<button>` (aria-label="Increase"), injected with the Phosphor `plus` glyph via `setIcon`, that increments the numeric value by `step` (clamped to max), emitting input + change. Also triggered by ArrowUp on the editor. Renders side-by-side with step-down in the trailing row — plus on the RIGHT (the − + order, GH #1406).
  - name: step-down
    description: Inside the trailing-adornment for type=number/currency/unit/percent. A square, icon-sized `<button>` (aria-label="Decrease"), injected with the Phosphor `minus` glyph via `setIcon`, that decrements the numeric value by `step` (clamped to min), emitting input + change. Also triggered by ArrowDown on the editor. Renders side-by-side with step-up in the trailing row — minus on the LEFT (the − + order, GH #1406, the iOS/Material stepper convention).
