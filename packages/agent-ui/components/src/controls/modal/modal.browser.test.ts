...
// ════════════════════════════════════════════════════════════════════════════════════════════════════
//  [7] GH #913 (#906's flagged adjacent gap) — the dialog part scrolls with a thin, auto-hiding scrollbar,
...
//  precedent). Focus is TRAPPED inside the dialog by showModal() (the platform), so the reveal-on-focus leg
//  checks the dialog part's OWN :focus-within — no sibling/host proxy needed (unlike command-modal's
//  nested-dialog case, whose search field lives outside the boundary :focus-within cannot cross).
// ════════════════════════════════════════════════════════════════════════════════════════════════════

describe('ui-modal cross-engine — GH #913: the dialog scrolls with an unobtrusive (thin) scrollbar, never the platform-default chunky bar', () => {
  it('overflows for real, scrolls, and computes the thin/reveal-on-open treatment (transparent-at-rest MEASURED on the closed dialog — see the next test\'s own note)', async () => {
    const { modal, dialog } = mount(
      `<ui-modal><div style="block-size: 3000px;">tall</div></ui-modal>`,
    )
    // BEFORE open — the dialog exists in the DOM (created at connect) but carries no [open] attribute, so
    // it is genuinely un-hovered and un-focused: the honest "at rest" baseline. MEASURED (both engines): a
    // native <dialog>'s "dialog focusing steps" ALWAYS place focus somewhere inside it once showModal() is
    // called (an autofocus/first-focusable descendant, else the dialog element itself, HTML §4.11.4) — so
    // once OPEN, :focus-within is unconditionally true and a post-open "not focused" snapshot is not
    // reachable by calling .blur() (a modal dialog's own focus cannot be relinquished to an inert page).
    expect(
    ...
    // This modal has NO focusable descendant, so showModal()'s own focusing steps land focus on the
  ...
  it('reveals the thumb while the dialog itself is :focus-within (focus is TRAPPED inside by showModal() — no sibling/host proxy needed)', async () => {
    const { modal, dialog } = mount(
      `<ui-modal><div style="block-size: 3000px;"><button>focus me</button></div></ui-modal>`,
    )
    modal.open = true
    await modal.updateComplete
    await raf()
