...
parts:                     # NOT shadow-DOM ::part() (light-DOM only) — light-DOM markers this element's own JS creates; documented for completeness (compareDescriptorToSource does not mechanically check `parts:`, the split.md/master-detail.md precedent)
  - name: settings-item
    description: One config-column section's FOLD, now spread across the Agent/Capabilities/Surface tabs (GH #574 — Kim's ruling, splitting the old single flat Settings tab; GH #225's Kim ruling, the GH #222 Context pattern applied back to the config column, is unchanged) — `<ui-disclosure data-part="settings-item" data-item="agent|model|surface|bankroll|prompt-section|skill|workflow|resource|tool|pattern-source|team">` (GH #1197 added `team` — the Team pane's own fold, `agent-team-pane.ts`), a chrome-free fold host whose summary IS the section heading (the shared heading register, chevron on the heading row) over the section's content card(s) as its body. ALL default OPEN (config is an editing surface — Context's newest-open logic is a log-reading choice, not a config one); built once, fold state lives in the live DOM, session-ephemeral. The Agent/kind folds carry their master switch ON the summary row via ui-disclosure's `slot="summary"` position slot (GH #226/ADR-0158 — a switch click never folds; the component owns the activation guard and the switch survives any fold rebuild). Replaces the old plain heading parts (agent-header/agent-heading/model-grid-heading/surface-options-heading/entry-section-header/-heading), all retired.
  - name: pane-holder
  ...
  - name: admin-header
    description: S7-c (admin-three-pane-ia.lld.md §16.1/§16.3) — `<div data-part="admin-header" data-slot="header">`, the `header` slot's one authored child (`#composeHeader`). Three zones in DOM order — `agent-select`, the pane-visibility pair (`pane-pills` wide / `pane-segments` narrow, one CSS band swap, never two copies of the truth), and `header-actions` — landmark role is the slot's own default (`banner`, super-shell.ts's `roleFor`; no `data-landmark` override, matching the anatomy's "no landmark override; the nav retired"). Paints at every band (unlike the retired nav, which used to hide itself wide — this element reuses that SAME line, re-derived from 54rem to 52.5rem by GH #1260 once the canvas gutter went to zero, to swap which of its two pane-visibility/actions renderings paints, never to hide the bar itself). Its own inline inset is `--ui-agent-admin-shell-gutter` (12px), the token's one surviving consumer besides the shell's `pane` slot — the pane holder beneath it is flush to the shell edge (GH #1260).
  - name: agent-select
    description: S7-c — `<ui-select data-part="agent-select">`, the roster picker `setAgentRoster`/`onAgentSelect` drive. Options are rebuilt wholesale on every `setAgentRoster` call (re-callable — a page re-pushes after a mint/import); the committed value is a silent programmatic write (`value =`, ADR-0019 — no `select` re-emission). The control's own internal `select`/`change` events stay contained (`stopPropagation`) — the closed seven-event set is untouched. GH #845 — the rebuild also re-composes the trailing `roster-actions` group at the panel's TAIL (see that part). GH #905 — each rebuilt roster option is ALSO minted with `aria-selected` from `activeId` (`"true"` on the active row, `"false"` on the rest): `ui-select`'s OWN selected mechanism, painted by select.css's `[role='option'][aria-selected='true']` rule, so the OPENED panel shows where you are. GH #908 update: `ui-select` now carries this reflection itself fleet-wide (a value-keyed sweep across every option on any `value` write, PLUS one at option-adoption time), covering the same rebuild-under-unchanged-`activeId` and no-click-programmatic-switch cases this page-level stamp names below — this mint-time stamp is therefore belt-and-braces, not the only writer (kept: synchronous, zero microtask-timing risk, provably harmless — both agree). Management items are excluded from the page's OWN stamp (see `roster-action`) but still receive `ui-select`'s own `"false"` sweep.
  - name: roster-actions
  ...
  - name: entry-header
    description: An entry's row. TWO shapes, by kind. The six drawered kinds (skill/workflow/resource/tool, GH #917, plus prompt-section/pattern-source, GH #949 — `entries.ts`'s `DRAWER_CRUD_KINDS`) render `[ entry-toggle | entry-label | entry-spacer | entry-badge? | entry-edit ]` — one affordance, opening the section's per-entry CRUD drawer. Only `catalog` keeps the inline shape `[ entry-toggle | entry-label | entry-spacer | entry-availability? | entry-rename? | entry-delete? ]` (the last three each opt-in; delete present only for a non-built-in entry).
  - name: entry-label
  ...
  - name: entry-delete
    description: An entry's remove affordance — a `<ui-button data-part="entry-delete">`, present ONLY for a non-built-in (custom) entry (TKT-0048; ADR-0132 Fork 4 — the protection is STRUCTURAL, never a disabled button). On an inline kind it sits last in the row; on a drawered kind it sits in the Edit drawer's FOOTER beside `entry-form-done` (GH #1063 ruling A, overturning #917's danger-row law) — destructive-styled by danger-token repoint and pushed to the far start (`margin-inline-end: auto`, entry-list.css), so it stays visually separated from the primary.
  - name: entry-edit
  ...
  - name: entry-edit-form
    description: GH #917 — the per-entry EDIT form inside `entry-drawer`'s content region (`entry-form.ts`, the same builder the add form runs through): name → description → tier (`entry-form-tier` wrapping the `entry-availability` control, built ONLY for a kind `hasAvailabilityMode` names — GH #949 widened the drawer to two kinds that never get one) → `entry-content` (+ `entry-content-count`); the delete affordance lives in the drawer footer (GH #1063), built only when the entry is not built-in. Every field commits on its OWN `change` through the same handlers the row used inline, so the footer's `entry-form-done` is a dismiss, never a batch Save. GH #947 — `entry-form-description`/`entry-add-description` carry no free-floating "Optional" line any more; their owning `ui-field` instead carries `data-optional`, which `entry-list.css` paints as a muted `· optional` suffix on the label itself.
  - name: entry-availability
    description: GH #947 — the Edit form's availability TIER control, a two-segment `<ui-segmented-control data-part="entry-availability">` ("In context" / "Invocable"), replacing the earlier `ui-toggle` pressed pill. Its sibling `entry-form-hint` line is now ONE dynamic sentence naming only the CURRENTLY selected state (not both at once). No writer (`onAvailabilityChange`) wired ⇒ the control renders `disabled` (a segmented control's `change` fires only after its own non-cancelable commit, so there is no pre-commit event left to refuse the way the old pill did).
  - name: entry-content-count
    description: GH #947 — a live character count (`<p data-part="entry-content-count" aria-live="off">`) beside `entry-content`/`entry-add-content`, updated on the editor's `input`. Purely informational — never gates the commit.
  - name: entry-form-title
    description: The drawer form's heading (`<h2>`) — the section's add label in add mode ("Add skill"). GH #947 — in EDIT mode this is now STATIC and kind-derived ("Edit skill"), never a live mirror of the entry's own name; the `entry-form-name` field is the one name source. GH #949 — the noun is derived from the section's own `addLabel` (stripping its leading "Add "), not the raw `kind` slug, so a multi-word or hyphenated kind reads as a sentence too ("Edit section" for prompt-section, "Edit pattern source" for pattern-source). Followed by `entry-form-tag` ("Built-in") for a protected entry, which STATES why that form builds no delete affordance.
  - name: entry-form-done
    description: The Edit drawer footer's primary ("Done") — it closes the drawer; every field already committed on its own change. GH #1063 (ruling A) — the footer also carries `entry-delete` for a non-built-in entry (the former `entry-form-danger` content block is gone): destructive-styled by the GH #947 danger-token repoint (`--ui-button-*` custom properties — never a fourth `ui-button` variant) and pushed to the far start, keeping the two actions visually separated.
  ...
  - name: entry-add-form
    description: A section's custom-entry authoring form. On an inline kind it is a permanently-mounted dashed card, hidden by default; on a drawered kind it is REBUILT FRESH on every open inside the drawer's content region — on close it isn't torn down, it merely remains inside the now-closed dialog until the next open replaces it via `replaceChildren`. Field order there is name → description → content — the ADD form carries no tier control (Phase 0 D3: a new entry is in-context by ABSENCE, SPEC-R1) and no Remove (GH #1063: Remove is the EDIT drawer footer's action). GH #950 — "rebuilt fresh" seeds from the section's own per-kind unsaved-draft buffer (`entry-list.ts`'s closure-scoped `addDraft`, kept live by every keystroke's `input`/`compositionend`), so an Esc/scrim dismiss followed by a reopen restores exactly what was typed; a successful Add clears the buffer.
  ...
  - name: surface-detail
    description: GH #541 — a modality's indented detail zone (`<div data-part="surface-detail">`), inset past its row's own left edge with a leading rule. Holds A2UI's catalog `entry-section` (roster + "+ From library" add-row, so both read as A2UI's) under its own `surface-detail-row` header (GH #844), and GenUI's `surface-detail-row` sub-option. Collapses to nothing when empty.
  - name: surface-detail-row
    description: GH #541 — a nested sub-option row inside a `surface-detail`: the same `[ switch | label ]` grammar as a modality row, on the group's shared inner surface rather than a card of its own. Exists so a modality row carries exactly ONE toggle scope — GenUI's "Use agent-ui components" used to ride the modality's own row as a second switch. TWO instances since GH #844 — `data-detail="genui-dogfood"` (`[ switch | label | surface-spacer | admin-help ]`) and `data-detail="a2ui-catalog"` (`[ label | surface-spacer | admin-help ]`, no toggle; GH #866 added both spacers so the icons reach the trailing edge like every other row): the catalog card is HEADLESS by construction (ADR-0170 cl.5 — no master switch, so no fold and no heading row of its own), which left its roster unlabeled under the A2UI toggle; it gets a header of this SAME shape rather than a bespoke one.
  - name: surface-detail-label
    description: A `surface-detail-row`'s own label (`<span data-part="surface-detail-label">`) — one part name for the one grammar, exactly as `surface-label` names every modality row's label alike. GH #844 renamed it from the row-specific `surface-genui-dogfood-label` when the detail zone gained its second labeled row; the old name was referenced by no CSS, no test, and not this block.
  - name: admin-help
    description: GH #844 (Surface tab) / GH #866 (admin-wide) — one explanation: a `<ui-tooltip data-part="admin-help" data-help="…" placement="bottom-end" delay="200">` whose ANCHOR is a focusable question-mark `<button>` and whose panel carries the `admin-help-card`. Mounted on every GROUP/SECTION header (`slot="summary"`, adopted onto the fold's heading row by ui-disclosure — ADR-0158) and every element row. `data-help` ∈ `ADMIN_HELP_KEYS` (agent-admin-schema.ts) — the Surface tab's nine (`surface-options · markdown · a2ui · a2ui-catalog · genui · genui-dogfood · planner · authoring · pattern-source`) plus GH #866's ten (`agent · model · bankroll · prompt-section · skill · workflow · resource · tool · context-agent · context-turn`). That table is also the ONE source of the copy: each row's native `title` hover hint is the SAME record's `summary`, so hint and card cannot drift, and one concept has ONE record wherever it appears (the Capabilities tab's `Skills` fold and the Context: System `Skills` item open the identical card — `helpKeyForKind`). PLACEMENT (Kim, 2026-08-14, two rulings): the icon rides the row's TRAILING edge — after the `surface-spacer` on an element row, after the flex-grown summary label on a heading row — and where that edge also carries a master switch the order is `[?] [switch]`, the switch outermost. The icon carries NO `data-part` of its own: `ui-tooltip` stamps `data-part="anchor"` on its first element child at connect (the `ui-menu` trigger precedent), so it is addressed scoped through this marker — `[data-part='admin-help'] > [data-part='anchor']`. That anchor is HIDDEN AT REST (`opacity: 0`, never `display:none` — it must stay focusable and keep its box) and revealed by hovering the row/heading it rides OR by focusing the icon itself; `role="tooltip"`, `aria-describedby`, immediate-on-focusin show, and focusout/Escape dismissal are ui-tooltip's own contract, never bypassed. A click on it is cancelled (`preventDefault`), so a header icon explains without folding the section it rides. `mountEntryList` is NOT involved: an entry-list section has been headless since GH #225, so its header is the consuming fold's summary and the opt-in is `helpKeyForKind` applied there — a bare entry-list mounts none.
  - name: admin-help-card
    description: GH #844/#866 — one help affordance's content plane (`<div data-part="admin-help-card">`), moved into `ui-tooltip`'s top-layer panel at connect. Plain structured markup, never markdown (the ruled default): a `admin-help-title` heading, a `admin-help-summary` gist line, one `admin-help-body` paragraph per expanded point, and an optional `admin-help-facts` list of labeled terms ("Default: Off"). Capped to a reading measure; the surface/frame/padding around it are `tooltip.css`'s, inherited unchanged. The `a2ui-catalog` card's facts are PROJECTED from `A2UI_CATALOG_OPTIONS` — registering a third catalog explains itself here with no edit.
  - name: bankroll-row
  ...
  - name: context-turn
    description: One logged turn — a context-item variant (`data-part="context-turn"`, the same GH #222 heading-row + one-card shape) whose JSON body is `{ arm: stub|live|surface, request, response }`; failures log too (`response.error`). The newest turn's fold defaults open. GH #866 — every turn's heading row carries the SAME `admin-help` icon (`data-help="context-turn"`): what needs explaining is the RECORD SHAPE and the retention bound, not one particular turn.
  - name: context-json
...
projects/restores losslessly. **GH #1260 (Kim's ruling, 2026-08-18) — edge-to-edge:** the three regions
meet the shell's edges and each other FLUSH — no canvas padding, no row gap (a deliberate reversal, by
ruling, of GH #686/#665's 12px gutter design: the gutters were the page-background "hairline" strips the
ticket was filed on). The seam between two painted regions is ONE hairline — `border-inline-start` on
every painted region after the first in `PANE_ORDER`, derived from `data-show`; the two `ui-conversation`
cards drop their own border + radius inside this composition (no doubled seams, no card border along the
shell's edges, no doubled header seam). This narrows GH #662's "no painted divider between top-level
regions" for that seam only — `ui-settings`' own nested rail|panel split still carries the no-divider law.
The header bar's content keeps its one-gutter inline inset above the flush holder. Standing pin:
`agent-admin-edge-to-edge.browser.test.ts` (both engines, 414×896 + 1280×800, dark + light).
...
`AgentRosterEntry` roster (`getKnownAgents`, read at invoke time), a dangling GM/member `agentId` flagged
visibly rather than dropped. **Surface** — how it renders: the Surface Options card (rev.6 — the
