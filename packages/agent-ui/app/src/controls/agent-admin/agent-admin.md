...
parts:                     # NOT shadow-DOM ::part() (light-DOM only) — light-DOM markers this element's own JS creates; documented for completeness (compareDescriptorToSource does not mechanically check `parts:`, the split.md/master-detail.md precedent)
  ...
  - name: surface-detail-row
    description: GH #541 — a nested sub-option row inside a `surface-detail`: the same `[ switch | label ]` grammar as a modality row, on the group's shared inner surface rather than a card of its own. Exists so a modality row carries exactly ONE toggle scope — GenUI's "Use agent-ui components" used to ride the modality's own row as a second switch. TWO instances since GH #844 — `data-detail="genui-dogfood"` (switch + label) and `data-detail="a2ui-catalog"` (label + help only, no toggle): the catalog card is HEADLESS by construction (ADR-0170 cl.5 — no master switch, so no fold and no heading row of its own), which left its roster unlabeled under the A2UI toggle; it gets a header of this SAME shape rather than a bespoke one.
  ...
  - name: surface-help
    description: GH #844 — one Surface-tab explanation: a `<ui-tooltip data-part="surface-help" data-help="…" placement="bottom-start" delay="200">` whose ANCHOR is a focusable question-mark `<button>` and whose panel carries the `surface-help-card`. Mounted on BOTH group headers (`slot="summary"`, adopted onto the fold's heading row by ui-disclosure — ADR-0158) and every element row (directly after the row's label). Nine of them, `data-help` ∈ `surface-options · markdown · a2ui · a2ui-catalog · genui · genui-dogfood · planner · authoring · pattern-source` — the key set is `SURFACE_HELP_KEYS` (agent-admin-schema.ts), which is also the ONE source of the copy: each row's native `title` hover hint is the SAME record's `summary`, so hint and card cannot drift. Surface-tab-scoped by ruling — the Capabilities tab's own kinds carry none (admin-wide is a named follow-up). The icon carries NO `data-part` of its own: `ui-tooltip` stamps `data-part="anchor"` on its first element child at connect (the `ui-menu` trigger precedent), so it is addressed scoped through this marker — `[data-part='surface-help'] > [data-part='anchor']`. That anchor is HIDDEN AT REST (`opacity: 0`, never `display:none` — it must stay focusable and keep its box) and revealed by hovering the row/heading it rides OR by focusing the icon itself; `role="tooltip"`, `aria-describedby`, immediate-on-focusin show, and focusout/Escape dismissal are ui-tooltip's own contract, never bypassed. A click on it is cancelled (`preventDefault`), so a header icon explains without folding the section it rides.
  - name: surface-help-card
    description: GH #844 — one help affordance's content plane (`<div data-part="surface-help-card">`), moved into `ui-tooltip`'s top-layer panel at connect. Plain structured markup, never markdown (the ruled default): a `surface-help-title` heading, a `surface-help-summary` gist line, one `surface-help-body` paragraph per expanded point, and an optional `surface-help-facts` list of labeled terms ("Default: Off"). Capped to a reading measure; the surface/frame/padding around it are `tooltip.css`'s, inherited unchanged. The `a2ui-catalog` card's facts are PROJECTED from `A2UI_CATALOG_OPTIONS` — registering a third catalog explains itself here with no edit.
  - name: bankroll-row
