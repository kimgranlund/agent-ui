...
parts:                     # NOT shadow-DOM ::part() (light-DOM only) — light-DOM markers this element's own JS creates; documented for completeness (compareDescriptorToSource does not mechanically check `parts:`, the split.md/master-detail.md precedent)
  ...
  - name: agent-select
    description: S7-c — `<ui-select data-part="agent-select">`, the roster picker `setAgentRoster`/`onAgentSelect` drive. Options are rebuilt wholesale on every `setAgentRoster` call (re-callable — a page re-pushes after a mint/import); the committed value is a silent programmatic write (`value =`, ADR-0019 — no `select` re-emission). The control's own internal `select`/`change` events stay contained (`stopPropagation`) — the closed seven-event set is untouched. GH #845 — the rebuild also re-composes the trailing `roster-actions` group at the panel's TAIL (see that part). GH #905 — each rebuilt roster option is minted with `aria-selected` from `activeId` (`"true"` on the active row, `"false"` on the rest): `ui-select`'s OWN selected mechanism, painted by select.css's `[role='option'][aria-selected='true']` rule, so the OPENED panel shows where you are. It is stamped here because the control's only writer of that attribute is `selectionCommit`'s commit-time reflect — the silent `value =` write moves the trigger label but marks no row — and because mint-time stamping is what survives a rebuild under an UNCHANGED `activeId` (a drawer rename/reorder re-push) and a switch with no click at all (boot/import/mint). Management items are excluded (see `roster-action`).
  - name: roster-actions
