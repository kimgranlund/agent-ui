...
parts:                     # NOT shadow-DOM ::part() (light-DOM only) — light-DOM markers this element's own JS creates; documented for completeness (compareDescriptorToSource does not mechanically check `parts:`, the split.md/master-detail.md precedent)
  ...
  - name: entry-edit-form
    description: GH #917 — the per-entry EDIT form inside `entry-drawer`'s content region (`entry-form.ts`, the same builder the add form runs through): name → description → tier (`entry-form-tier` wrapping the `entry-availability` control, built ONLY for a kind `hasAvailabilityMode` names — GH #949 widened the drawer to two kinds that never get one) → `entry-content` (+ `entry-content-count`), then `entry-form-danger` when the entry is not built-in. Every field commits on its OWN `change` through the same handlers the row used inline, so the footer's `entry-form-done` is a dismiss, never a batch Save. GH #947 — `entry-form-description`/`entry-add-description` carry no free-floating "Optional" line any more; their owning `ui-field` instead carries `data-optional`, which `entry-list.css` paints as a muted `· optional` suffix on the label itself.
  ...
  - name: entry-form-danger
    description: GH #917 — the Edit form's separated danger block (a hint plus `entry-delete`), the LAST child of the drawer's scrolling content. Structurally absent for a `builtin` entry. GH #947 — `entry-delete` here carries danger styling (a token repoint to the danger-family ink/border, `entry-list.css`'s own DANGER-INTENT-BY-REPOINT rule, `--ui-button-*` custom properties — never a fourth `ui-button` variant), scoped to this drawer row only; the teaching-line hint is unchanged.
  ...
  - name: entry-add-form
    description: A section's custom-entry authoring form. On an inline kind it is a permanently-mounted dashed card, hidden by default; on a drawered kind it is REBUILT FRESH on every open inside the drawer's content region — on close it isn't torn down, it merely remains inside the now-closed dialog until the next open replaces it via `replaceChildren`. Field order there is name → description → content — the ADD form carries no tier control (Phase 0 D3: a new entry is in-context by ABSENCE, SPEC-R1) and no danger row. GH #950 — "rebuilt fresh" seeds from the section's own per-kind unsaved-draft buffer (`entry-list.ts`'s closure-scoped `addDraft`, kept live by every keystroke's `input`/`compositionend`), so an Esc/scrim dismiss followed by a reopen restores exactly what was typed; a successful Add clears the buffer.
