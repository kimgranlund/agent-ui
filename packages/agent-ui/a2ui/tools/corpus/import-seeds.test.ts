...
describe('dispositionGuard — GH #335 defect 1 (an unjudged run must not silently re-admit a disposition-allowlisted candidate)', () => {
  ...
  it('the REAL allowlist is the default input — every shelf name in it halts, every other shelf name passes through (GH #1353 flipped this from the prior all-empty steady state)', () => {
...
describe('import-seeds main() — the verdict archive (ADR-0165), real subprocess runs against a sandbox repo root', () => {
  ...
  it('the sandbox itself is honest — an unjudged run over an EMPTY sandbox corpus either admits the whole shelf (an empty real allowlist) or halts on the first real disposition (GH #1353)', () => {
    ...
    // test. Between 2026-08-18 and GH #1353 the real `DISPOSITION_ALLOWLIST` was legitimately EMPTY (the
    // standing refusals were dropped, the pending backlog admitted), so an unjudged run with no planted
    // archive and no allowlist entry admitted the whole shelf. GH #1353 (`frontier-drill-settings`)
