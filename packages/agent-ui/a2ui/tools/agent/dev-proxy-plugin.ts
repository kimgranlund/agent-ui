        ...
        void (async () => {
          ...
          try {
            ...
            if (req.method === 'POST') {
              ...
              // ADR-0182 cl.2 / SPEC-R31 — the SAME fail-closed validation both transports share
              // (chat-validation.ts): anything but a literal boolean degrades to `undefined`, which
              // composes ZERO builder-mission teaching bytes. Absent ⇒ the opts key is omitted entirely
              // and the composed prompt is byte-identical to a pre-#716 turn.
