...
// site-preview-catalog.test.ts — the drift gate for the <component-preview> element's TWO hand-authored maps
// (site/lib/component-preview.ts). The element's knobs + variant chips + the catalog page's component list are
// all DERIVED from the shipped default catalog, so they cannot drift; but two small maps are hand-keyed by
// catalog COMPONENT NAME and are exactly where the hand-authored half can fall behind the catalog:
...
// Placed in the a2ui package (inside the vitest packages-only `include`) because the catalog is the source of truth.
