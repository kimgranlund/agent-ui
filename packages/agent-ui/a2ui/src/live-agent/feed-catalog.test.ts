...
describe('feed sub-catalog partition gate (ADR-0097 §3 / SPEC-R15 / LLD-C14)', () => {
  it('has the exact ADR-0097 §3 counts — 37 IN, 43 OUT (11 at ratification + the chart-family pair, ADR-0097 Amendment / ADR-0107 Amendment 2 + the report/content/feed catalog wave\'s 3 IN / 5 OUT, ADR-0111/0113/0112 + the token-surface family\'s 0 IN / 3 OUT, ADR-0118 cl.6 + the M4 app-surfaces panes wave\'s 0 IN / 2 OUT, ADR-0120 cl.5 + the toolbar wave\'s 1 IN / 0 OUT, ADR-0121 F7 + the timeline-family wave\'s 0 IN / 2 OUT, ADR-0122 F5 + the swiper-family wave\'s 0 IN / 2 OUT, ADR-0124 F5 + the color-picker wave\'s 0 IN / 1 OUT, ADR-0123 cl.6 + the form-popover wave\'s 0 IN / 1 OUT, GH #294 F4/form-popover.spec.md SPEC-R9 + the 2026-07-28 per-control catalog intake\'s 1 IN / 0 OUT, Textarea + the ADR-0163 M-A table-widening wave\'s 0 IN / 1 OUT, saas-data-workbench.spec.md SPEC-R4, Pagination + the M-F multi-select-field wave\'s 1 IN / 0 OUT, multi-select-field.lld.md/ADR-0175, MultiSelect + the ui-drawer wave\'s 0 IN / 1 OUT, ADR-0188 cl.2, Drawer + the toast-outcome wave\'s 0 IN / 1 OUT, GH #1184, Toast + the description-list receipt wave\'s 1 IN / 0 OUT, ADR-0201/GH #1185, DescriptionList — the confirm-step receipt is the ask card\'s own idiom + the image-content wave\'s 0 IN / 1 OUT, GH #1189, Image — a URL-sourced photo is report/artifact content, not an ask affordance + the line-chart wave\'s 0 IN / 1 OUT, ADR-0205 cl.8/GH #1207, LineChart — the chart family\'s third member, the same display-only reasoning as Sparkline/BarChart + the media-players wave\'s 0 IN / 2 OUT, GH #1209, Video/AudioPlayer — URL-sourced players are content, the Image reasoning verbatim + the GH #1353 Drill-catalog-decision wave\'s 0 IN / 2 OUT, ADR-0195 GH #954, Drill/DrillPanel — an arbitrary-depth tree hiding all-but-one panel, no value mark to commit either + the GH #1352 Toggle-catalog-decision wave\'s 0 IN / 1 OUT, ADR-0179 GH #686 Amendment S7-a — no value mark at all, worse than the already-excluded Switch + the 2026-08-19 nine-ADR campaign\'s 6 IN / 1 OUT, ADR-0210/0213/0214/0216/0220, FileDrop/Suggestions/SourceList/Rating/ChoiceGroup/ChoiceCard IN + ADR-0219, PieChart OUT — the chart family\'s fourth member, the same display-only reasoning + the ADR-0224 ServiceCard wave\'s 0 IN / 1 OUT, GH #1429, ServiceCard OUT — a gateway service-list card is report/dashboard content, and its Open action does not round-trip to the server this pass + the GH #1515 Breadcrumb wave\'s 1 IN / 0 OUT, Breadcrumb IN — the DescriptionList/SourceList static-informational-furniture parity argument, not the ServiceCard/Table report/canvas-scale exclusion class + the GH #1568 svg-charts wave 4\'s 0 IN / 2 OUT, ADR-0229 cl.7, ColumnChart/Gauge OUT — the chart family\'s fifth/sixth members, the same display-only reasoning as Sparkline/BarChart/LineChart/PieChart)', () => {
    expect(FEED_SURFACE_TYPES.length).toBe(37)
    expect(FEED_EXCLUDED.length).toBe(43)
  })
