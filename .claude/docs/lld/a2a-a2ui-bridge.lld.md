...
> Implements: [`../spec/a2a-foundations.spec.md`](../spec/a2a-foundations.spec.md) **SPEC-R16** (+ consumes SPEC-R2 pin · SPEC-R6/N4 shared validator · SPEC-R8 `A2aChannel` loopback · SPEC-N1/N2 split/hygiene — referenced, not re-derived). PRD trace via the SPEC: **PRD-G5** (bridge); the demo surface also serves **PRD-G4**'s posture (drift-gated site section). Contract ownership: the adapter's *contract* stays the a2ui family's ([`../spec/a2ui-streaming-pipeline.spec.md`](../spec/a2ui-streaming-pipeline.spec.md) **SPEC-R5**) — this LLD realizes it, adds no second contract (acceptance defers to a2ui SPEC-R5 AC1, per SPEC-R16).
...
- **Live-arm seam — REALIZED by B7 (2026-07-09, [`a2a-live-realtime.lld.md`](./a2a-live-realtime.lld.md) LLD-C9…C11, ADR-0116):** as reserved here, the live arm landed as `probeFeedLive()` + a DEV-guarded dynamic `import()` of `feed-live-transport.ts` (the a2ui-live/arena overlay pattern, exactly the posture this row named), the composer genuinely sending over the new `/__a2a/feed` mount. The recorded default and everything else in this LLD are unchanged — the seam held its shape.
