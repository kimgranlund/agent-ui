...
| **LLD-C4** | Geometry + tokens + states CSS — §7's table; the editor overlay (grid-area spanning all tracks, `opacity` on text `0`/`caret-color: transparent`, still focusable/hit-testable); cell states (empty/filled/active/disabled/user-invalid) on the entry family's color roles; forced-colors + `[density]` participation per `interaction-states.md`; the echo/message nodes' visually-hidden recipe. | `controls/otp-field/otp-field.css` | §7 |
| **LLD-C5** | Descriptor — `otp-field.md` frontmatter: tag/class/tier `control`/props/events (`input`,`change`)/parts (`editor`,`cell`,`message`,`echo`)/the geometry row contract. | `controls/otp-field/otp-field.md` | §2 |
