...
| **LLD-C7** | Integration trio + site surfaces — `controls/index.ts` barrel, `component-styles.css` `@import`, `package.json` exports subpath; docs doc/demo pages + gallery specimen; the `naming.md` §12 recorded-exception row for the owner-accepted `otp` acronym (§2 Tag row). | per `component-build` step 8 | §2 Site row |
