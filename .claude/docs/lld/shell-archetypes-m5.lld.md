...
**Design.** Rename the reflected props/attributes to logical `collapsed-start`/`collapsed-end`, `narrow-start`/`narrow-end` (CSS logical-property vocabulary, matching `inline-start`/`inline-end` the fleet already uses elsewhere for the same reason). The DOM's own `data-side="left"|"right"` markers on the rail/pane parts stay PHYSICAL (that's what CSS `[data-side='left']` selectors need to target real boxes), but which physical side counts as "start" now resolves through `:dir(rtl)`/`:dir(ltr)` in the CSS, not through a hard-coded `left`/`right` JS mapping — `#makeToggle`'s narrow-arm click handler reads `getComputedStyle(this).direction` (or `this.matches(':dir(rtl)')` where supported) to decide which physical side a logical toggle click affects. Toggle `aria-label`s become "Toggle start panes"/"Toggle end panes".
