...
|---|---|---|---|
| LLD-C1 | `IntegrationManifest` + `ExecuteContext` types, the registry core — `registerIntegration()` (boot-fail-fast on duplicate `id`/wire-name, on a schema beyond C2's subset, and on a `serverKey` manifest declaring no `envKey` — SPEC-R18), `listIntegrations()`, `resolveIntegrations(ids, env)` (intersect on ids · non-array ⇒ `[]` · cap 16 · exclude unprovisioned `serverKey` manifests) | NEW `packages/agent-ui/a2ui/tools/agent/integrations/registry.ts` + `registry.test.ts` | SPEC-R16, R18 |
...
| LLD-C7 | admin label decoupling — `INTEGRATION_TOOLS` entries display the manifest's human `label` and carry the registry `id` explicitly: `NewEntryInput` gains an optional `id` (trimmed, never slugged — it is a foreign key; blank falls back to `slugify(label)`), so the id rides ON the entry through the real add path and both wire sites project `entry.id` via a shared `#enabledToolIds` helper — no site-side lookup table exists (as built: none can, given the package/site layering); the preset seeder is a SECOND pack→entry projection under the same `e.id ?? e.label` law; the parity test (`agent-admin-app.test.ts`) widens from bare labels to `{id, label, description}` trios against the real registry (node-importable, as today) *(cell amended at build time: the shipped mechanism replaced the drafted page-local trio-table projection, S5 review PASS)* | `site/pages/agent-admin-libraries.ts` · `site/pages/agent-admin-presets.ts` · `packages/agent-ui/app/src/controls/agent-admin/entries.ts` + `entries.test.ts` · the wire sites in `packages/agent-ui/app/src/controls/agent-admin/agent-admin.ts` · `site/pages/agent-admin-app.test.ts` | SPEC-R16 AC2 |
