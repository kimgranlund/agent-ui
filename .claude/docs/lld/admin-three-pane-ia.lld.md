...
└── ui-chat-shell                       (the vehicle stays — ADR-0154/ADR-0179; resizable-end/narrow-end retire)
    ...
    └── [data-slot="content"] div[data-part="pane-holder"]        (was chat-stack)
        ...
        └── ui-master-detail[data-part="pane-pair"]                visible ⇔ pane ∈ {author, settings}
            ...
            │       └── [data-part="log"]
            │           (GH #681/GH #684, 2026-08-10: empty ⇔ authoringStore unset — no
            │            "author-empty" node exists any more, see §2's OQ4 amendment row)
...
*(GH #686, 2026-08-10 — RATIFIED, and BUILT S7-a→S7-e (confirmed 2026-08-11): this anatomy is
superseded by §16.1's — the header slot's contents change and the MD row dissolves into three
...
- **`authoringStore` cleared while on Author:** the card returns to its unarmed, empty-log state
  (always-present, OQ4); the interview transcript tears down under the shipped identity-change law,
  not this LLD's. **AMENDED 2026-08-10 (GH #684)** — "empty state paints" no longer means a
  dedicated node: the log is simply empty (§2's OQ4 amendment row).
...
- **No callback registered on `onGenerateRequest`:** the card stays `disabled` (static-build
  degrade, §2 OQ4). **AMENDED 2026-08-10 (GH #681 + GH #684)** — the prior text here ("the empty
  state's action hides; copy still names the flow") is stale on both counts: the action was removed
  by GH #681 and the copy by GH #684, so there is no action to hide and no copy left to name the
  flow — only the `disabled` composer itself signals "no mint path" (§2's OQ4 amendment row).
...
| **LLD-P4** | Author place: `author-empty` + the `onGenerateRequest` registration seam; the conversation's new mount target. *(GH #666, 2026-08-10: `author-empty` became the card's empty-LOG state and the mount stopped being lazy — see the Author-region row in §2.)* *(GH #681 + GH #684, 2026-08-10: `author-empty`'s action then its headline + copy were removed — the node itself is gone, see §2's OQ4 amendment row.)* | `agent-admin.ts` (+ `agent-admin-app.ts` registering the callback → `createGeneratedAgent`) | S1-b |
...
- **No MD API change** — back suppression is admin CSS; the drill-band seam is the named
  escalation, built only if S3-a's evidence demands it. ⚠️ **MOOT (RATIFIED; BUILT S7-a→S7-e, confirmed 2026-08-11) 2026-08-10
...
- **No pane persistence, no URL/router binding** (the router stays catalog-invisible; the admin is
  ...
  changes (ADR-0132/0164/0170 untouched). ⚠️ **SUPERSEDED IN PART (RATIFIED; BUILT S7-a→S7-e, confirmed 2026-08-11) 2026-08-10
...
1. The three-place nav is live at every band from ONE vehicle; the six-entry narrow-tabs
   vocabulary is gone (cl.1). ⚠️ **SUPERSEDED (RATIFIED; BUILT S7-a→S7-e, confirmed 2026-08-11) 2026-08-10 (GH #686) — the nav vehicle
...
2. `#contextFor` is keyed by active pane via §2's frozen algorithm — the selector diff is the one
   ...
   to composer ORIGIN (§2's origin row); GH #686 (RATIFIED, built) inherits origin-keying untouched, §16.2.**
...
4. The wide pairing is the SAME section nodes arranged (identity-asserted), docked at the 40rem
   ...
   both engines (cl.3; #651's acceptance line). ⚠️ **SUPERSEDED (RATIFIED; BUILT S7-a→S7-e, confirmed 2026-08-11) 2026-08-10 (GH #686) —
...
6. "New agent → Generate" — from the roster menu AND the Author empty state — lands the user in
   ...
   ⚠️ **SUPERSEDED (found stale by S7-e's own record-repair pass, 2026-08-11) — the roster (...)
   menu this item names is GH #686's own canvas-header, RETIRED whole (S7-d); the two arming
   entries are now the header's single "New Agent" button (`onNewAgentRequest` → Generate, S7-d)
   and the Co-pilot place's always-present empty log (unchanged in substance). "New agent → Blank"
   — this item's own OTHER shipped mint path, distinct from the roster menu it names — is RETIRED
   entirely, Kim's OQ-A ruling (§16.6, GH #686 Findings 2026-08-11): Generate is the only "new
   agent" verb.**
...
## 16 · Amendment — the unified header + shown-set visibility (GH #686, 2026-08-10, **RATIFIED 2026-08-10, built S7-a→S7-e — confirmed 2026-08-11**)
...
> [#686](https://github.com/kimgranlund/agent-ui/issues/686) Amendment — the
> unified-header ruling (Kim's Figma wireframe: desktop `1:162`/`1:163`, mobile `1:502`); cited by
> issue number, never by ordinal. This section is the build-ready OUTLINE (slice grain +
> seam shapes + retirement map); each slice's full per-slice acceptance detail lives in its own
> build (S7-a…S7-e's own Findings, GH #686's comment thread) rather than a rewrite of this outline.
> Contradictions with the accepted body escalate, never silently edit.
>
> **2026-08-11 correction (S7-e record-repair pass).** This heading and every dated ⚠️ marker
> elsewhere in this file that read "(proposed) 2026-08-10 (GH #686)" were accurate when written
> (2026-08-10, design pass, before ratification) but went STALE the moment Kim ratified the
> amendment later the same day
> ([utterance](https://github.com/kimgranlund/agent-ui/issues/686#issuecomment-5242385245),
> ADR-0179's own amendment heading) and then stale a second time as S7-a→S7-d actually built it
> (GH #686's comment thread carries each slice's Findings). Every such marker in this file now
> reads "(RATIFIED; BUILT S7-a→S7-e, confirmed 2026-08-11)" in place of "(proposed)" — a dated
> correction, not a silent rewrite; the ORIGINAL "(proposed)" framing was correct FOR ITS OWN DATE
> and is preserved in git history. ADR-0179's own amendment banner carried the identical staleness
> ("no build dispatches until Kim rules") — corrected in the same change, `0179-agent-admin-three-pane-ia.md`'s own dated addendum.
...
- **OQ-A — New Agent's verb — RULED 2026-08-11 (Kim, via the host, AskUserQuestion; recorded on GH
  [#686](https://github.com/kimgranlund/agent-ui/issues/686)'s own thread as a dated "## Ruling —
  OQ-A" comment).** RETIRE the "New agent → Blank" mint path entirely — Generate becomes the ONLY
  "new agent" entry point, through the header's single New Agent button (`onNewAgentRequest`).
  `mintBlankPersona`'s own mint LOGIC stays in the codebase (Generate still reuses it verbatim for
  its own seed); only the standalone Blank UI entry point (GH #637 S1's dedicated front door) is
  gone. S7-d had already shipped exactly this outcome PROVISIONALLY (flagging the choice for this
  ruling rather than guessing at OQ-A's other two candidate homes — the narrow `•••` menu or an
  ADR-0170-style pack action); this ruling confirms it as decided, no rework owed.
