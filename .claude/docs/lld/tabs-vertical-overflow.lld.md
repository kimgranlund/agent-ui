...
| Commit path + event containment | tabs listens for the menu's `select` ON the `ui-menu` element, calls `stopPropagation()`, maps `data-value` → full-list index, and runs the existing `#commit(index,…)` → ONE tabs `select`. The menu's `toggle`/`close` are contained the same way | `emit()` bubbles+composed (verified `element.ts:263`): uncontained, the menu's `select {value,index}` would reach the A2UI renderer's `value:{prop:'selected',event:'select'}` binding and write a menu value into the data model. `ui-tabs`' event surface stays exactly `{select}` |
| Measurement | one `ResizeObserver` on the strip (guarded `typeof ResizeObserver !== 'undefined'` — jsdom; fit behavior is browser-suite territory). Available = the strip's content-box INLINE-size (horizontal) / BLOCK-size (vertical). Reserve = `--ui-tabs-tab-height` resolved + one gap — token-derived, never measured from the trigger. Fit: if Σ(cached sizes) + (N−1)·gap ≤ available ⇒ K=N, menu part hidden; else K = max k with selected pinned + prefix fitting alongside the reserve; floor K=1 (the selected tab always renders; below that the strip clips — recorded edge) | gap read from computed style at fit time (density-live); reserve needs no DOM measure |
...
- **Browser, both engines (LLD-C12):** fixed 320px strip with wide tabs → K visible + trigger visible +
  ...
  Escape from open menu → focus on trigger; vertical: strip beside panel (whole-shape bounding boxes,
...
- **Chat-shell at compact** (GH #575's owner): whether the shell adopts `overflow="menu"` instead of
  ...
  `overflow` default.
- **Width-cache staleness** (late font swap after `fonts.ready`, ancestor `[density]` font effects): the
  fit may run on slightly stale sizes until the next reconnect; consequence is one tab too many/few, not
  oscillation (inputs stay render-independent). Accepted; recorded in `tabs.md` if observed.
