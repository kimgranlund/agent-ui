...
| **Try-it toggle anatomy** (S4 builds) | A `data-part="try-it"` bar atop the chat stack, visible only while `authoringStore` is set: two `ui-button`s ("Authoring" / "Try it"), `aria-pressed` managed, calling `#setMode(...)`. No new host events (`#setMode` is internal; the closed event set is untouched). | Smallest fleet-native affordance for a two-state visibility flip; S4 refines visuals against the geometry law without re-opening this contract. |
...
- **Mode.** `#mode: 'authoring' | 'test'` — component state, meaningful only while
  ...
  `aria-pressed` — nothing else: no store touch, no reset, no serialization. Both transcripts
  survive by never unmounting (the round-trip acceptance).
