...
> single-slot array value, association/relationship editing fenced out). Author: planner (design
> seat).
>
...
| **LLD-C3** | The checkmark anatomy — **no new attribute at all.** Corrected from v0.1's first draft, which mis-keyed this as a `data-role="check"` adornment: a control-painted glyph tied to the control's OWN reflected state is `data-part`/no-attribute anatomy territory (ADR-0157 cl.1/cl.4 — `data-part` is OWNER-stamped chrome, declared in `parts:`; `data-role` is the ADR-0012 AUTHOR-supplied content-kind axis for adornments the consumer places in a slot, the wrong axis here since nothing is author-placed). Rather than minting a `data-part="check"` identity for a glyph that is not a distinct chrome NODE (no new element, no new slot — it is a pseudo-element paint keyed to an attribute the control already reflects), this drops the new channel entirely: painted **CSS-only** as `[role=option][aria-selected="true"]::before`, reading directly off `aria-selected` (which `selectionCommit` already reflects onto each option, `selection-commit.ts:70-77` — unchanged) with zero new anatomy vocabulary — the presumptively-right fix under ADR-0157's "no NEW component mints a competing anatomy channel" clause. `aria-hidden` by construction (the glyph never carries meaning; `aria-selected` does). Sized `= font` (an inline AFFORDANCE, rhythm family, the §4.1 caret law — NOT `--ui-ind`, the named oversize bug class), centered in an icon-sized leading cell (§1.4/§1.5's slot-presence pad model — same mechanics a trailing caret already uses, just leading position; no new sizing mechanism). Zero DOM injection into author/catalog-adopted option nodes — matches `ui-checkbox`'s own "paints entirely in CSS `::before`, no shadow DOM" precedent, and respects that `[role=option]` children are authored/adopted light-DOM the control does not want to mutate structurally. | `controls/multi-select/multi-select.css` | SPEC-R8 |
| **LLD-C4** | `traits/selection-commit.ts` — add `'multi-toggle'` to `SelectionMode`. New arm: BOTH the click handler and the Enter-keydown handler unconditionally toggle the targeted item's Set membership, ignoring Shift/Ctrl modifiers entirely (no range-extend, no separate toggle-vs-replace branch — every commit path is "toggle this one item," full stop). Required because the EXISTING `'multi'` mode's plain click **replaces** the whole selection (`multiKeys = new Set([key])`, `selection-commit.ts:135` — the file-manager/`<select multiple>` convention) while its Enter handler's multi-mode branch already toggles (`:162-169`, the `else` arm past `:158-161`'s single-mode branch) — an internal click/Enter asymmetry that fails SPEC-R3's "plain click adds, never replaces" outcome AND SPEC-R8's "keyboard matches pointer byte-for-byte" contract if reused as-is. Additive only: `'single'`/`'multi'` stay byte-untouched (a regression probe in `selection-commit.test.ts` proves it) — this is the one piece of shared trait code the build touches outside the new folder. | `traits/selection-commit.ts` + `selection-commit.test.ts` | SPEC-R3, R8 |
...
at its third either — flagged, not required, §12). Each option needs a `value` attribute (the
...
8. Standing trip-wires pass (import-layering, naming/structure, descriptor↔`static props`↔source,
   zero-native + internals-ARIA, geometry/token checks) — the create procedure's validation-loop
   step 2, no new migration needed (this is a NEW tag, not a rename).
9. **`npm run size`, by hand** (the create procedure's validation-loop step 3 — manual by Kim's
   ruling, since the bundle surface changed: a new control + a new catalog factory arm) — confirm
   the marginal size lands inside the `tier: pattern` composite's budget, tree-shake clean.
10. **`node scripts/generate-llms-full.mjs`** — regenerate the llms artifacts (the create
    procedure's step-8 leg), since this build changes a descriptor + (at merge) the CHANGELOG.
11. **Independent `component-checker` hand-off** (the create procedure's validation-loop step 4 —
    non-optional, both rubric axes ≥ 4; generator ≠ critic, the same law this LLD's own doc-checker
    review already applied at the design layer) — fix the component, not the check, on any finding.
12. `roadmap.md` §3's M-F line restates to "shipped" at merge time — ADR-0175's own Repairs cell
