...
The `Category`/`LABEL`/`categoryOf`/`narrateCategories` functions from `a2ui-chat.ts` move into `conversation.ts` unchanged (they are already pure, presentation-only, zero-dep helpers over raw JSONL strings — a straight promotion, not a rewrite).

**Correction (M2 build wave, empirically verified against both consumers).** An earlier draft of this note wrongly listed `narrateTrace`/`TurnTrace` alongside the category helpers as "promoted unchanged." It is **NOT** promoted. The ratified SPEC §4 `AgentTurnHandle` exposes exactly four methods (`ingestLine`/`setNote`/`finalize`/`fail`); **none lets a caller supply a `TurnTrace`**, so there is no reachable call site for `narrateTrace` under the frozen contract, and adding a fifth handle method would invent public API the SPEC never asked for. Neither consumer actually needs the primitive to narrate a trace: `a2ui-live.ts` never feeds a trace to any render surface — its `TurnTrace` is consumed page-side by `traceDigest()` as prompt-shaping on `TurnInput.text` (unchanged, stays app-owned); `a2ui-chat.ts`'s `narrateTrace` was a **live-arm-only, untested** informational status entry (`meta.trace` is absent on the shipped recorded transcript, so it never fired in any gate). Its drop on migration is a live-only presentation delta, not a contract regression — flagged for a future design ruling if a consumer ever needs trace narration inside the primitive (which would widen `AgentTurnHandle`). The element never assumes a trace exists — because it never sees one.
