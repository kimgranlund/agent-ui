...
| **Events** | NONE of its own — crumb activation is native anchor navigation; the composed `ui-menu`'s `select`/`toggle`/`close` are composed-part internals (the tabs posture: commit relays, nothing re-emitted). Zero new event names; the closed seven untouched | navigation is the platform's event; minting a `select`-per-crumb would duplicate `<a>` semantics |
...
| **Router-link composability** | ADR-0115-BLIND BY CONSTRUCTION, and composable anyway: crumbs are tag-agnostic light-DOM children, so `<a href>`, `<ui-router-link>` (which stamps a real inner `<a>` — relay hits it via the `querySelector('a')` arm), and plain `<span>` interchange freely; `ui-breadcrumb` never imports, names, or sniffs `@agent-ui/router` (the components→router edge is illegal anyway — `layering.test.ts`) | composability falls out of the mechanism; no edge, no allowlist, nothing to fence |
