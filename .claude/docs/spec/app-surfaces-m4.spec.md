...
- **`ui-split-pane`** — the generic pane child of `ui-split` (the slider-thumb precedent: the control renders the interactive separators, the author authors only the panes). Carries the per-pane constraint props (`min`/`max`, initial `initial` (the seed prop — renamed from `size` at build: the fleet A2 gate reserves that name, review-ratified)).
...
- **AC5** *(1:1 tracking, no compounding — TKT-0015)* *Given* an in-flight drag, *when* **two or more** sequential `pointermove`s are dispatched, *then* each move's resulting ratio matches the delta measured from the drag's ORIGINAL press-time baseline — a single large move and N small moves covering the same net pointer distance MUST land at the same final ratio; the assertion bites if N small moves overshoot past what the net travel warrants (the pre-fix double-counting).
...
**SPEC-R5 — Touch targets, focus ring, forced-colors, reduced-motion, selection.** The separator's **interactive hit area MUST be ≥ 24×24 CSS px** (WCAG 2.5.8) even when the visual divider is 1–2px — a transparent hit-slop centered on the divider, without shifting the pane layout. The focused separator MUST show the fleet focus ring (interaction-states law). Under **`forced-colors: active`** the divider + focus indicator MUST remain visible (real border/`currentColor`, no fill-only affordance that vanishes). The control MUST NOT animate resize by default (respect `prefers-reduced-motion`; any optional transition is off during active drag). **While a drag is active, text/element selection MUST be suspended across the WHOLE split — not just the separator — so a fast sweep never selects pane content; selection MUST be restored immediately on drag end (commit) or an aborted mid-drag pane mutation (SPEC-R2 AC6) (TKT-0015).** *(→ PRD-G7, PRD-G6)*
...
- **AC4** *(selection suspension, TKT-0015)* *Given* an in-flight drag, *when* `user-select` is read on the host AND on a pane's content, *then* both resolve `none`; *when* the drag ends (release OR an aborted mid-drag pane mutation), *then* selection is restored — the assertion bites if only the separator (not the whole split) suspends selection, or if an aborted drag leaves it stuck suppressed.
...
interface UISplitPaneElement {
  initial?: number                             // initial ratio seed (uncontrolled) ; min/max = CSS length props
