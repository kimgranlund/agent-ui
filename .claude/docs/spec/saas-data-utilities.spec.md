...
> Status: accepted · v0.2 · 2026-08-22 · Layer: SPEC (execution contract)
> **v0.2 revision (2026-08-22) — reconciliation against the shipped package, shipped reality wins** (the same reconciliation class as the parent PRD's v0.2, commit 0cb7f04a): (1) every "ADR-0192 proposed — Kim flips" reference updated — ADR-0192 was **accepted 2026-08-16** (Kim's `ratify ADR-0192` utterance on PR #996, executed by `scripts/adr_ratify.py`); (2) the R1 consumer edge brought current — [ADR-0227](../adr/0227-context-shared-state-grammar-and-data-adoption.md) (accepted 2026-08-20) activated the cl.1-reserved `app → data` edge in its adoption waves 1–2 (wave 1 = its clause 4; wave 2 = its dated Amendment); (3) §7's wave ledger updated to record both waves as shipped; (4) the ADR-0191 citation updated proposed→accepted. No requirement, AC, or non-goal is re-litigated — the shipped build conforms to §3 as written (`paginated()` was already a first-class `.` export in R1/R7).
> Refines: [`../prd/saas-data-utilities.prd.md`](../prd/saas-data-utilities.prd.md) — **PRD-G1** (DataSource seam), **PRD-G2** (read pathway), **PRD-G3** (mutation pathway), **PRD-G4** (streaming), **PRD-G5** (gateway client), **PRD-G6** (pagination), **PRD-G7** (fleet pillars), **PRD-G8** (teaching); realizes [ADR-0192](../adr/0192-agent-ui-data-package.md) clauses 1–6 (**accepted 2026-08-16** — v0.2 update, was "proposed, NOT yet ratified" at v0.1). Rules the PRD's delegated **PRD-D6** (PRD §5 — defaults + budget) as recommendations on record, in this SPEC's §3.5 and the clauses it names (R10, R12, R14).
...
- **Design wave:** PRD/SPEC/ADR authored; `npx vitest run site/lib/adr.test.ts site/lib/docs-grammar.test.ts` exit 0; `coverage_check.py --strict` exit 0 on the manifest; `docs:doc-checker` verdicts on all three docs repaired to PASS. *(v0.2: discharged — ADR-0192 accepted 2026-08-16; v0.1's "Status stays `proposed` until Kim ratifies" hold is kept here as the record of the sequencing law it enforced.)*
- **Build wave (on ratification):** every R-AC above green; `npm run check && npm test` + the `./stream` browser shard exit 0; the manifest's leaves closed in edge order. *(v0.2: shipped 2026-08-16/17 — `packages/agent-ui/data/**` with the `.`/`./gateway`/`./stream` surfaces, layering + headless gates, and the size line-item; #955/#956/#957 closed. The substantive dogfood then exceeded R14(f)'s one-page metric via ADR-0227's shared-state-grammar adoption — see the PRD's v0.2 note and `../references/state-and-persistence.md`.)*

## Agent verification

An agent verifying this SPEC checks it where the checks already live — nothing here is restated:
every requirement in §3 carries its own inline acceptance criteria (each **SPEC-R#**'s AC1..ACn are
the per-clause predicates, realized as named tests under `packages/agent-ui/data/src/**`), and §7 is
the wave-level harness that rolls them up (design-wave doc gates; build-wave `npm run check && npm
test` + the `./stream` browser shard, judged by exit code). A claim of conformance cites the
specific SPEC-R# AC, not this section.
