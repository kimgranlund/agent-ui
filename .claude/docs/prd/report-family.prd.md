...
- **Table interactivity** — ***AMENDED 2026-07-28 by [ADR-0163](../adr/0163-ui-table-interactive-widening.md)** (proposed): **sorting, row selection, filtering/search, and pagination LEAVE this fence** and are ADMITTED to the family's contract (report-family SPEC §3.6, SPEC-R21…R28) — the re-entry arrived by its own record, exactly as this fence's "each re-entry is a new intake" clause demanded (Kim's ruling, roadmap §3 M-A). Every capability defaults OFF and the defaults-on rendered DOM stays byte-identical to the display-only control.* Still fenced under this bullet: **virtualization, column resizing, editable cells**, plus (named by ADR-0163 cl.1) filter OPERATORS beyond the facet shape, an in-table query input, `sort-mode="manual"` server-side sort, row-click selection, and a live results-count `aria-live` region — each drags focus/keyboard/state machinery that converts a display type into an application widget (the axis-system analog of ADR-0107's fence — where table scope explodes).
