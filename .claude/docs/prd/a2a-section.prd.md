...
> Status: **accepted · v0.4 · 2026-07-09 · Owner: agent-ui** — awaiting Kim's ratification of the five forks (§5). Authored by the system-planner seat; independent doc-review requested before ratification. (v0.4: **§6 gains B7 (live real-time demo arms)** at the [ADR-0116](../adr/0116-a2a-live-realtime-examples.md) intake — Kim's 2026-07-09 ask *"can we make the A2A examples live so they occur in real time user initiated?"*; serves the EXISTING goals G2/G5/G4, no new goal — behavior in SPEC-R17/R18 (§4.6 widening, pending doc-review), design in [`../lld/a2a-live-realtime.lld.md`](../lld/a2a-live-realtime.lld.md). v0.3: assumption **A-3 re-judged at B6 intake** — the bridge's double block has dissolved; §2 G5 / §4 A-3 / §6 B6 / §7 re-synced, evidence in [`../lld/a2a-a2ui-bridge.lld.md`](../lld/a2a-a2ui-bridge.lld.md) §2; re-judgment pending doc-review. v0.2: §6 B0-gate wording re-synced to the fully verified HV-1…HV-12 ledger.)
...
> Location note: filed under `.claude/docs/prd/` per the ratified location charter (agent-app-surfaces PRD-D6); the A2UI family was unified onto this same map (repo-alignment, 2026-07-12).
> Grounding: Kim's verbatim goal (2026-07-07): *"create a A2A section that is on par of excellence with A2UI work — I want demos where one agent can play another agent in tic tac toe, under purely separate contexts and no cross contamination — I want a well documented corpus of A2A concepts and demos."* · The one A2A seam already spec'd: [`../spec/a2ui-streaming-pipeline.spec.md`](../spec/a2ui-streaming-pipeline.spec.md) **SPEC-R5** (carry A2UI over A2A; capabilities in A2A `Message.metadata`) + [`../lld/a2ui-streaming-pipeline.lld.md`](../lld/a2ui-streaming-pipeline.lld.md) **LLD-C5** (`tools/pipeline/transports/a2a.ts`, dev/server-scoped, **unbuilt**) · Reuse substrate: the a2ui live-agent stack (provider seam, bounded produce loop, recorded-default + dev-proxy security posture — ADR-0069–0073) and the a2ui corpus discipline (facets · admission · pins · standing data gate).
...
- *Baseline*: no section. · *Target*: **three** pages live (arena · concepts · the B6 artifact-feed — Kim's 2026-07-08 ratified feed shape, the widening SPEC-R16 AC2 realizes), gates standing. · *Timeframe*: **B5** (arena + concepts) · **B6** (artifact feed).
...
- *Metric*: a2ui pipeline SPEC-R5 AC1 + transport-invariance smoke + a WORKING demo surface (the artifact-feed page: A2A messages whose `application/a2ui+json` DataParts render as live inline A2UI surfaces — SPEC-R16 AC2).
- *Baseline*: LLD-C5 unbuilt. · *Target*: smoke green. · *Timeframe*: **B6** *(the cross-family dependency §4 A-3 originally named has dissolved — A-3's re-judgment)*.
...
- **A-3** *(RE-JUDGED 2026-07-08 at B6 intake — the original block has dissolved; pending doc-review)* — Originally: the bridge (PRD-G5) additionally depends on a2ui pipeline LLD-C1 (codec) + LLD-C3 (transport abstraction), both unbuilt, so B6 was blocked until they landed. Re-examined against the shipped tree: **(a)** the producer the bridge carries exists — the live-agent wave realized the streaming family's LLD-C2 loop (`a2ui/tools/agent/produce.ts`, generate→validate→self-correct, its header says so verbatim); **(b)** the JSONL codec (LLD-C1) is not on the bridge's path — HV-8's resolution pins carriage as one A2UI envelope per A2A `DataPart` as *structured JSON* (no line framing on the A2A wire), and the shipped renderer host ingests per-line at the seam; **(c)** the transport abstraction (LLD-C3) is not on the bridge's path — the carrier is `@agent-ui/a2a`'s own shipped channel (SPEC-R8), and a2ui SPEC-R5's observable contract (capabilities in metadata, order) is checkable without that family's plug-adapter seam. **B6 is unblocked on shipped surfaces.** Full evidence: [`../lld/a2a-a2ui-bridge.lld.md`](../lld/a2a-a2ui-bridge.lld.md) §2.
...
- **B6 — Bridge.** A2UI-over-A2A realization of pipeline LLD-C5 + the artifact-feed demo page (Kim-ratified demo shape, 2026-07-08). Unblocked at intake (A-3's re-judgment); designed in [`../lld/a2a-a2ui-bridge.lld.md`](../lld/a2a-a2ui-bridge.lld.md). *Gate: a2ui SPEC-R5 AC1 smoke (SPEC-R16 AC1) + the standing feed-fixture gate with biting negative controls (SPEC-R16 AC2, widening pending doc-review).* *(PRD-G5)*
- **B7 — Live real-time demo arms** (intake 2026-07-09, [ADR-0116](../adr/0116-a2a-live-realtime-examples.md) proposed). The arena's dev-only live match streams move-by-move into the shipped replay UI (cancel = abort + discard; the isolation verdict derives from the COMPLETED transcript via the same checker), and the artifact-feed's reserved live arm becomes a real user-initiated conversational turn loop over A2A (caps server-verified; the client-held A2A log is the session; prose + artifacts render progressively). Recorded-default (C4) and the ADR-0073 trust boundary are unchanged — both arms stay dev-only. Designed in [`../lld/a2a-live-realtime.lld.md`](../lld/a2a-live-realtime.lld.md). *Gate: SPEC-R17/R18 ACs — chunk-equivalence + frame round-trip + same-checker-over-live-artifacts legs (offline, stub provider, truncation negatives biting) + the dist grep over both mounts — **met 2026-07-09** (both slices built; the arena arm additionally proven with a real key-configured dev run: a genuine Sonnet-vs-Haiku match streamed move-by-move, plus a clean mid-match client disconnect).* *(PRD-G2, G5, G4)*
...
- [`../lld/a2a-a2ui-bridge.lld.md`](../lld/a2a-a2ui-bridge.lld.md) — the B6 LLD: the bridge mapping (HV-8 carriage + capabilities), the round-trip smoke, the recorded artifact-feed fixture + standing gate, and the artifact-feed demo page. *(landed 2026-07-08)*
- [`../lld/a2a-live-realtime.lld.md`](../lld/a2a-live-realtime.lld.md) — the B7 LLD: the streaming arena arm (incremental accumulator, runner abort seam, follow-tail replay) + the feed's live conversational arm (part-frame protocol, log-as-session derivation, `/__a2a/feed` proxy, page overlay), one LLD for both slices. *(landed 2026-07-09 with ADR-0116, proposed; keeps the SPEC trace closed — SPEC-R17/R18)*
