...
- Same-wave default-catalog rows (`Sparkline`, `BarChart`, `PieChart`) + the report-card exemplar seed (PRD-G3) — forced by the SPEC-N2 whole-fleet gate the moment the descriptors land. `ColumnChart`'s own catalog row is a named FOLLOW-UP wave (ADR-0229 cl.7) — the control ships this wave with a named allowlist entry, never a rider.

**Out of scope (v1) — the fence, each with its reason:**
- ~~**Axis systems**~~ **(ticks, gridlines, scale labels, legends)** — where chart scope explodes; the v1 types are chosen precisely because neither owes an axis. **Realized, scoped to `ui-column-chart` only (v1.2, ADR-0228/ADR-0229, ADR-0107 Amendment 4)** — every other axis-bearing type (scatter, log/dual axes, a typed time axis, `ui-gauge`, the `axes`-state `ui-line-chart` widening) remains a **new intake or a later wave of the same ADR-0229**, never a rider.
...
- **Scatter, line-with-axes, multi-series, stacked/grouped bars** — all drag axes, legends, or series-color systems (the same explosion). ~~Stacked/multi-series COLUMNS~~ **realized (v1.2, `ui-column-chart`, ADR-0228/ADR-0229)** — `ui-bar-chart` itself stays exactly as fenced (axis-free, no stacking, its own foreseen `orientation` flip untouched); scatter, line-with-axes (beyond `ui-line-chart`'s own realized `axes` state, a LATER wave of ADR-0229), and grouped (non-stacked) bars remain fenced.
