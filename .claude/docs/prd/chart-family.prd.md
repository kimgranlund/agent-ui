...
- ~~**Axis systems**~~ **(ticks, gridlines, scale labels, legends)** — where chart scope explodes; the v1 types are chosen precisely because neither owes an axis. **Realized, scoped to `ui-column-chart` (v1.2) AND `ui-line-chart`'s `axes` state (v1.3) — ADR-0228/ADR-0229, ADR-0107 Amendment 4** — every other axis-bearing type (scatter, log/dual axes, a typed time axis, `ui-gauge`) remains a **new intake or a later wave of the same ADR-0229**, never a rider. Legends stay fenced for every type (the `ui-column-chart`/`ui-line-chart` chip vocabulary is not a legend).
