...
This PRD is refined by the following SPECs (what must be built / how it behaves) and LLDs (how it is built). Status lives in each doc's own header (the tree wins — `agent-ui-doc-standards` §2); decisions in [`../adr/`](../adr/) (the numbered files ARE the index — no index file, `agent-ui-doc-standards` §1b); the original charter table is [archived](../archive/a2ui-expert-system/README.md) (frozen 2026-07-08).
