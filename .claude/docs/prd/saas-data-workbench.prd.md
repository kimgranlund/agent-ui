...
|---|---|---|---|
| **PRD-D1** | What does "SaaS UI patterns" deliver? | **Three tiers; the page is the proof, not the product** | **ratified** — Kim, 2026-07-31 |
| **PRD-D2** | The demo's data + persistence shape | **Seeded in-repo fixture; in-memory, in-session persistence; no fetch layer** | **ratified** — Kim, 2026-07-31 |
| **PRD-D3** | Which shipped control carries the multi-value facet? | **`ui-form-popover` + a check group**, page-side aggregation | **ratified** — Kim, 2026-07-31 |
| **PRD-D7** | Does the workbench sheet join the AC19 gate's set? | **Yes — one reviewed append at MA-3** | **ratified** — Kim, 2026-07-31 |
...
**MA-1 and MA-2 are fully independent and start in parallel.** Their file sets are disjoint: **MA-1** touches `packages/agent-ui/components` (table, the new pagination folder) + `packages/agent-ui/a2ui` (catalog rows) + the **report-family SPEC/LLD/PRD** docs its ADR's Repairs cell owes; **MA-2** touches `packages/agent-ui/app` only, plus the **`composition-patterns` skill** (ADR-0164 cl.5's two rows). The only shared *neighbourhood* is `.claude/docs`, and even there the files differ (report-family vs none). Nothing in the manifest connects them. Running them serially buys nothing.
