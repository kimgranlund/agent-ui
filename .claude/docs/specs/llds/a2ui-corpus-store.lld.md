...
> Implements: [`../specs/a2ui-training-corpus.spec.md`](../specs/a2ui-training-corpus.spec.md) (SPEC-R1..R16, SPEC-N1..N6). Closes **PRD-D4** (storage substrate); PRD-D5 (MCP delivery) is now served through the streaming-pipeline LLD (see LLD-C13 note, §1).
...
| Deferred (designed, trigger-dispatched) | C8 (first eval record) · C11 (first pin bump) · C12 (harness wave) | §10/§11 carry the designs; §13's deferred tail names the triggers |
...
| **LLD-C12** | Scoring + lift harness (Inspect-AI interop) | SPEC-R14, R15, R16 | `tools/corpus/eval/` | dev/CI | unbuilt · deferred to the harness wave (rides harness LLD-C3 rubrics + LLD-C6 loop) |
...
Deferred tail (designed above, dispatched by their own triggers): **LLD-C8** contamination (trigger: the first eval record) → **LLD-C11** repair loop (trigger: the first pin bump) → **LLD-C12** scoring/lift (trigger: the harness wave, NEXT item 3) → MCP exposure (owned by streaming LLD-C6).
