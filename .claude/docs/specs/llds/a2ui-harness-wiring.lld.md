...
| **LLD-C3** | Rubric files | SPEC-R3 | `.claude/docs/specs/rubrics/a2ui-*.md` |
| **LLD-C4** | Deterministic gate scripts/hooks | SPEC-R4 | `packages/agent-ui/a2ui/tools/gates/` + CI + pre-commit |
| **LLD-C5** | Orchestration wiring | SPEC-R5 | frontmatter + `.claude/docs/specs/rubrics/a2ui-artifact-map.md` |
...
| routing miss | LLD-C1 | trigger eval < 90% → tune skill `description` via `skill-author` (SPEC-N2) |
...
.claude/docs/specs/rubrics/  a2ui-payload-quality.md  catalog-quality.md  corpus-quality.md  a2ui-artifact-map.md
...
2. **LLD-C3 rubrics** — author via `rubric-author`; each passes the rubric gate; `corpus-quality` wired as corpus tier-2. *(checkpoint: rubric gate green)*
3. **LLD-C1 skills** — author via `skill-author`; run the trigger eval. *(checkpoint: routing ≥ 90% — SPEC-N2)*
4. **LLD-C2 agents** — author via `agent-author`; each names its rubric, scoped tools, model. *(checkpoint: `harness_checks.py agent` green; reference artifact ≥ 4/5 — SPEC-N4)*
