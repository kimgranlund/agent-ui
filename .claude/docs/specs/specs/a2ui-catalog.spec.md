...
| `Toolbar` | `ui-toolbar` | **shipped** (ADR-0121 F7, toolbar.spec.md SPEC-R1..R12/toolbar.lld.md LLD-C1..C10). A `role="toolbar"` content-region arrangement — `elevation`/`brightness` (surface, ADR-0015) + `orientation`/`align`/`justify`/`gap`/`overflow` (the arrangement axes, structural, NOT bindable — the `Row`/`Column` flex-grammar precedent) are 1:1 reflecting accessor props; bindable `label` (the author-supplied accessible name → `internals.ariaLabel`, the `Icon.label`/`Avatar.label` accessible-name precedent, not a light-DOM textContent mapping). No `value` mark and no events of its own (SPEC-R5 — the toolbar arranges + roves keyboard focus over its light-DOM children, it is not a command bus; item children emit their own commits). `ChildList` children — host-as-flex, the `Row` precedent: light-DOM children ARE the roving item set (`ui-button`/native `button`/`a[href]`/`[role=button]`/`[data-toolbar-item]`), one Tab stop, arrow keys move focus with no wrap. **In the feed sub-catalog** (`FEED_SURFACE_TYPES`, ADR-0097 §3 widened by ADR-0121 F7) — the `Row` parity argument: an arrangement of the same action `Button`s a `Row` already hosts in a feed ask, adding only `role="toolbar"` + roving focus, no overlay/pagination/dashboard shape. **Row-vs-Toolbar usage guidance:** reach for `Toolbar` when the children are a cluster of command/action controls that should read as ONE bar with a single Tab stop and arrow-key navigation between them (a document's action row, a formatting palette); reach for `Row` (the List/Grid guidance's own "a toolbar" example, still valid for a non-interactive or mixed-content arrangement) when no such roving-focus semantic is wanted |
