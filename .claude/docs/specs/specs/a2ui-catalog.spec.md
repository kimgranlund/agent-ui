...
| `Row` | `ui-row` | **shipped** (G9, ADR-0016; `reflow` added ADR-0096). `elevation`,`brightness` (surface, ADR-0015); `align`,`justify`,`gap`,`wrap` (flex grammar); `reflow` (`auto` default·`locked` — gates the ADR-0016 cl.4 `@container` narrow→column switch; element-local, not part of the flex grammar); `ChildList` children |
| `Column` | `ui-column` | **shipped** (G9). As `Row`, column main axis (direction is the tag's identity, not a prop). `reflow` (`locked` default·`auto`, ADR-0096) — the per-tag default FLIPS relative to `Row`: a column no longer switches to a row under a wide container unless `reflow="auto"` is set, so the catalog's prop-only consumer can reach and pin the tag's own vertical identity |
...
> residue remains (Wave D reconciles/confirms this, the code allowlist is already empty).
