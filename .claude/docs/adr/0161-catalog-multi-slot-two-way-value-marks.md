...
> | **Ratified by** | Kim Granlund (repo owner), 2026-07-28 — the owner-authored Status flip in commit [`7ffaad5`](https://github.com/kimgranlund/agent-ui/commit/7ffaad5ef5d78d890221d1a087e7feba5e5362a4). Citation back-filled 2026-07-28 from that commit's git evidence: the flip was made by hand, and `adr_ratify.py` (which writes both cells together) was not the path, so this cell still read `pending` afterwards. |
