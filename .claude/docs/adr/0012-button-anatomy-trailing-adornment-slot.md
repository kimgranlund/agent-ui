...
5. **Family standard.** This `position × role` convention — positional slots for layout, a `data-role` axis for
   ...
   (`ui-select`'s caret, a field's leading icon, a chip's badge) reuse it, recorded in `component-author`.
