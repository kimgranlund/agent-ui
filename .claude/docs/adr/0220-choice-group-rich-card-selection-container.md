...
**What already exists to build on.** `controls/_base/listbox-element.ts` (`UIListboxElement`,
ADR-0042 / listbox-roving LLD-C3) is the shipped selection base: form-associated, `role=listbox`
via internals, a `multiple` prop flipping single (string) vs multi (`ReadonlySet`) selection,
`required → valueMissing`, and `rovingFocus` + `selectionCommit` wired over its option children.
`selectionCommit` (verified at source) commits on real user gestures (click/Enter — explicit
activation, never selection-follows-focus), emits `select` on commit, and parameterizes both
`items()` and `keyOf()` — exactly the seams a card-shaped option family needs. The missing piece
is a thin family, not a new interaction model — the "minting is cheap when it is" check
(mint-vs-compose reference) applies verbatim.
...
1. **Identity & class.** `UIChoiceGroupElement extends UIListboxElement` (the shipped base carries
   value/validity/mode/roving/commit); `UIChoiceCardElement` is the option unit — a light-DOM
   card-shaped host whose ENTIRE surface is the hit target, carrying option semantics via
   `ElementInternals` (never host attributes — the fleet ARIA law; the ui-tab precedent for
   internals-driven selection reflect layered over the trait, per `selection-commit.ts`'s own
   GH #908/#905 header contract). Tag→type is mechanical PascalCase (no alias row needed in the
   fleet-derived coverage gate). Names derive from the family name `choice` per `naming.md` §13.
2. **One control, two modes.** `multiple: boolean` (structural, reflected — the `UIListboxElement`
   contract) flips single (exclusive, radio-like) vs multi (toggle — `selectionCommit`'s shipped
   `multi-toggle` mode, the `multi-select` precedent). ADR-0095's T1 fact — a multi-select
...
5. **Keyboard/ARIA: listbox semantics, both modes, explicit activation.** `role=listbox` on the
   group (`aria-multiselectable` in multi), `role=option` + `aria-selected` on cards (internals);
   ...
   (radiogroup vs nothing-for-multi) and drag selection-follows-focus in with them. A missing
   Space leg in the shared trait is a build-time re-verify, not a contract change.
...
7. **ADR-0212 posture: aligned-by-construction, NOT dependent.** `ui-choice-group` discovery is
   born nearest-group-scoped-descendant (every `UIChoiceCardElement` descendant whose nearest
   choice-group ancestor is THIS group; an inner group is an ownership boundary), wired through
   `selectionCommit`'s own `items()`/`keyOf()` seams. This is the same rule ADR-0212 proposes
...
Both types enter `FEED_SURFACE_TYPES` (ADR-0097 §3): a rich-option pick IS the canonical
...
- **On ratification+build (exit gates; per-slice gates in the decomposition):** jsdom — single
  ...
  negative control, disabled cards skipped by roving and never committed; browser — selected
...
- **`UIListboxElement` gains its first non-popup, non-field consumer.** If the base's trait
  wiring resists the card family at build (the `items`/`keyOf` override, the internals-vs-
  attribute reflect split, a missing Space leg), the builder escalates per the ADR-0095
  subclass-vs-extract precedent rather than forcing it — a build-time re-verify, not a fork for
  Kim.
- **Stale → re-verify on build:** §5.2 (the drafted rows) · `feed-catalog.ts` counts ·
  `choosing.ts` GROUPS routing (a "pick from rich options" arm beside the `Table.selectable`
  guidance) · the prompt inventory + one frontier seed · this ADR's Repairs cell.
...
- **Two types (`ChoiceGroup` + `MultiChoiceGroup`).** Rejected: `UIListboxElement` already
  unifies the modes under `multiple`; two types double the vocabulary and the teaching surface
