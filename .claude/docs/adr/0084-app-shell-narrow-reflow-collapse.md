...
> | **Supersedes / Superseded by** | Resolves LLD §7.5's reserved narrow-reflow item. Relates ADR-0082 (the isolated `:host` grid variant must mirror this rule). No ADR superseded. **Superseded by (partial) [ADR-0156](./0156-deprecate-app-shell-for-super-shell-family.md)** *(the `ui-app-shell-region` `collapse` mechanism retires with the component — the reserved `toggle` member drops with it; the narrow-reflow PATTERN — the declarative per-X reflow enum with a stay-visible `stack` arm (ADR-0156 clause 3's own wording: super-shell reflows per-side, it has no regions) — lives on as family law in the super-shell grammar: ADR-0156 clause 3 · REV 2026-07-26, the mechanical pointer repair per ADR-0156 clause 5)*. |
