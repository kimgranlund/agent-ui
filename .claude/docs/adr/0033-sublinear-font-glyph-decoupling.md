...
> | **Supersedes / Superseded by** | **Supersedes ADR-0007 — the font/glyph linear-derivation leg ONLY.** ADR-0007's headline decision (declare the derived ramp tokens on the universal `*` so subtree `[scale]`/`[density]` re-substitute) **STANDS**, and the **`--ui-height` linear leg** (`base × var(--ui-scale)`) stood at the time *(later superseded by ADR-0038 — `--ui-height` becomes Kim's explicit `(scale × size) → §1-row` lookup; see below)*. What this ADR reverses is the *font/glyph value formula*: `--ui-font = base × scale` (linear) → `base × pow(scale, 0.45)` (sublinear). Per the ADR-log rule, reversing a mechanism is a supersession (a new ADR), so this supersedes that sub-decision; mark ADR-0007 *"font/glyph leg superseded by ADR-0033."* **Relates ADR-0032** (corrects how its `[scale]` ladder feeds glyphs) and **ADR-0025** (`--ui-type-*` stays linear). · **Font/icon `pow` MECHANISM superseded by ADR-0035** — Kim ruled control fonts must be the **exact §1 SET integers**, so the `pow` approximation (decimals like 20.6) is replaced by an explicit per-tier §1 table; **the sublinear/§1-faithful OUTCOME STANDS** (ADR-0035 realizes the same intent, exactly). · **Control ramp superseded by ADR-0038** (ADR-0037 folded in) — Kim ruled *"let's not use multipliers"*, so `--ui-{height,font,icon}` become an explicit `(scale × size) → §1-row` lookup (height + font + icon from ONE §1 row, no `× --ui-scale`); clause 1's *"height stays linear"* AND this ADR's `× pow(scale)` font framing are both history (the font is now an explicit lookup). `--ui-scale` survives for `--ui-type-*` display only. |
...
1. **Height stays linear (ADR-0007 height leg, UNCHANGED).** `--ui-height-{sm,md,lg} = calc(base × var(--ui-scale))` (base 24·28·36). The frame box is the lever; it scales 1:1 with `[scale]`. *(History — **superseded by ADR-0038**: `--ui-height` (with font/icon) comes from Kim's explicit `(scale × size) → §1-row` lookup, no `× --ui-scale`. The frame-is-the-lever framing stands; only the value mechanism reverses.)*
...
- **S2 — `dimensions.test.ts` (source ripple).** The font assertions (`calc(13|14|16px * var(--ui-scale))`) → the `pow(var(--ui-scale), 0.45)` form. Add a source assertion that height stays linear (`× var(--ui-scale)`, no pow) *(superseded by ADR-0038 — controls drop `× --ui-scale` for Kim's explicit lookup; that assertion flips there)* and `--ui-type-*` stays linear (the fork). *(Source-structural only — the real px proof is the browser gate.)*
