...
> | **Supersedes / Superseded by** | None. **Extends ADR-0047** (the numeric family) into the date/time family; composes the Wave-4 overlay. **Extended by ADR-0105** (the day-cell ramp here survives as the fluid track FLOOR and the point-layer circle diameter — its square-cell/circular-state anatomy is re-expressed in two paint layers, not replaced; this record's grid/keyboard/form Decision stands unchanged). **Extended by ADR-0093** (its `range = reserved/future` scope note is resolved there — the reserved `--ui-calendar-range-*` tokens are realized and the `mode` flag lands on this record's own control; this record's single-date Decision stands unchanged). |
