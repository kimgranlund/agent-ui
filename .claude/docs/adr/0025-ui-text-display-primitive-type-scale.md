...
> | **Supersedes / Superseded by** | **Superseded by ADR-0078 — in part** ([`0078-ui-text-three-axis-variant-size-as.md`](./0078-ui-text-three-axis-variant-size-as.md)): cl.1's one-`variant` prop schema (→ the three axes `variant`×`size`×`as`) · cl.3/3a's `--ui-type-*` scale (→ `--md-sys-typescale-*`) · cl.4's internals-heading mechanism (→ real-element stamping). The Display-leaf primitive (cl.1), slotted content (cl.2), and the catalog `Text` type (cl.5) **STAND** — this ADR stays accepted. · Relates: **ADR-0006** (host-as-content / void `render()` — the slotted-text mechanism cl.2 reuses) · **ADR-0015** (the new-token-family precedent — `--ui-space`/`--ui-radius-base`; `--ui-type-*` follows its `*`-ramp-vs-`:root`-constant law) · **ADR-0024** (the dynamic-list demo whose leaf content this fixes) |
