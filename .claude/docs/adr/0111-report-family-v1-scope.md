...
1. **The v1 type set** *(PRD-G1)*: `ui-table` (a **static data table** — typed columns, record rows,
   ...
   > **Forward pointer — amended 2026-07-28 by [ADR-0163](./0163-ui-table-interactive-widening.md)**
   > (proposed): table **sorting, row selection, filtering/search, and pagination** left this
   > ruled-out list and are admitted to the contract (report-family SPEC §3.6, SPEC-R21…R28), and a
   > standalone `ui-pagination` was minted. The re-entry arrived by its own record, exactly as this
   > clause demanded. Everything else on the fence — virtualization, column resizing, cell renderers,
   > interactive chips, anchored count-dots, a stat child seam, delta valence coloring, plus the
   > operators/in-table-query-input/server-side-sort items ADR-0163 cl.1 names — **stays fenced**.
   > This record's Status is unchanged (**accepted**); only its cl.1 list is narrowed.
