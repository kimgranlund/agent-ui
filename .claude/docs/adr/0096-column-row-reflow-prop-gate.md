...
> | **Supersedes / Superseded by** | **Extends ADR-0016** (clauses 1/2/4 all stand — the flex grammar, direction-as-tag-identity, and container-query responsiveness survive; this gates *when the identity may adapt* behind a prop and re-bases one default on evidence 0016 never had) · relates ADR-0030 (per-consumer default precedent) · ADR-0075 (element-local prop, not `flexProps`) · ADR-0076 (enum honoring makes the new enum load-bearing) · ADR-0071 (derived inventory advertises it) · ADR-0091 (the idiom-teaching lane) · **Generalized by ADR-0102** (the CSS-less-consumer contract law promotes this record's no-CSS-verb forcing argument to a fleet-wide decision procedure — reciprocal link landed at 0102's acceptance, per its scheduled Repairs) · **Extended by ADR-0100** (the boundary-establishment law — this record's gated reflow rules ride unchanged, resolving against boundary containers) |
