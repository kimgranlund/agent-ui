...
> | **Ratified by** | kimgranlund (repo owner), 2026-08-21, via the [`ratify ADR-0230` utterance](https://github.com/kimgranlund/agent-ui/issues/1578#issuecomment-5371504880) — verified + flipped by `scripts/adr_ratify.py` (ADR-0149). **Clause-6 fork: branch B (producer-side granularity) picked** — named in the utterance itself; note hand-appended by the host in the flip commit per this record's own capture mechanism (cl.6). |
