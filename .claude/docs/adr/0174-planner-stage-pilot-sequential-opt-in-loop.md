...
> | **Ratified by** | kimgranlund (repo owner), 2026-08-06, via the [`ratify ADR-0174` utterance](https://github.com/kimgranlund/agent-ui/pull/495#issuecomment-5203384531) — verified + flipped by `scripts/adr_ratify.py` (ADR-0149) |
...
(ADR-0138, `produce.ts:112-114`'s own doc comment) is appended AFTER the catalog/grammar law, with
an explicit precedence rule: persona voice/content, never wire authority. `GRAMMAR`
(`system-prompt.ts:67`, `a2ui-live-agent.spec.md` SPEC-R6 AC2/AC3) is the host-owned pole this
precedence rule protects — byte-pinned, drift-gated by `system-prompt-grammar.test.ts`, and it is
where the TWO most recent new model-facing mechanics blocks landed, both AFTER `GRAMMAR` already
existed: ADR-0097 §4 (2026-07-07) added the `ask`-arm's mechanics teaching, and ADR-0126
(2026-07-11, chronologically the later of the two) added the message-lifecycle decision-layer
teaching — the four-type choice rule + `deleteSurface` wire shape (`prompts/grammar.md:73-77`,
credited at `system-prompt.ts:30-31`). Both landed in `GRAMMAR`, neither in a persona-editable
`kind: "prompt-section"` entry (ADR-0132 cl.2's Instructions primitive) — two independent
confirmations of the same precedent, not one. The `plan` arm (cl.2 above) is structurally the SAME
kind of fact both `ask` and the message-lifecycle rule are — new wire syntax/behavior the model must
reproduce exactly for the protocol to keep working — so it follows the identical precedent: **the
`plan`-arm mechanics teaching (how to declare a plan, and the synthesis-turn's own procedural
framing) lands in `GRAMMAR`, host-owned, never persona-editable.**
