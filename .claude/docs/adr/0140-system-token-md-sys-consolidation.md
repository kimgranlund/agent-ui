...
> | **Repairs** | on ratification+build: `shared/src/tokens/dimensions.css` + `base.css` (the declarations) · every fleet consumer of the ~34 shared names (~150 files) · `naming.md` §5/§12 (the foundation-tier canon + the 2026-07-12 ruling's supersession note) · `dimensions.test.ts`/`styling-gates.test.ts`/`family-coherence.test.ts` (the gates) · the ADR-0038 geometry tables' prose realized in `geometry-sizing-spec.md` (verified done — **not** `adr-0038.md`'s own Decision-clause/Repairs-cell prose, which this repair never touched; see #1681) · the theme-provider built-CSS fixture |
