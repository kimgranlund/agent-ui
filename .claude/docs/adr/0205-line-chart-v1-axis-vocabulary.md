...
> | **Supersedes / Superseded by** | (none) — **Extended by [ADR-0229](./0229-svg-chart-family-extensions.md)** (cl.3, accepted 2026-08-20, realized GH #1566 "svg-charts wave 2" — the reciprocal backlink: ADR-0229's own header already carries `Extends ADR-0205`. Cl.3 realizes this ADR's own cl.7 gridlines/ticks/category-axis later-list, via the new opt-in `axes` state consuming the shared ADR-0228 `_chart/` vocabulary; AND this ADR's cl.3 "min/max labels are always shown" law becomes **default-state-scoped** — the tick-label chips carry the axis values once `axes` is set, so the two systems never render together. This ADR's own Decision stands unedited; the extension builds on it, per doc-standards §1b's Extends↔Extended-by pointer shape) · relates [ADR-0107](./0107-chart-family-v1-scope.md) (names this exact gap: *"axis systems ... line-with-axes, multi-series ... Any axis-bearing type is a new intake"* — this ADR is that named intake, and leaves ADR-0107's own v1 scope for `ui-sparkline`/`ui-bar-chart` untouched) |
