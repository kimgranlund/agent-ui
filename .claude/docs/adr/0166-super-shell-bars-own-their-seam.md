...
1. **A bar's surface is the SAME token as a card's.** `--ui-super-shell-surface` is
   ...
   separator does not read as "flush" — it reads as one L-shaped surface blob wrapping the
   transparent canvas.
...
inline axis is untouched — middle's gaps, its cards' inline radii, and GH #214's resizer arithmetic
all stand verbatim.** Eight clauses.
...
so the footer has no symmetric spelling. `:has()` on the frame is symmetric, and the **`> ` child
combinator is mandatory** in both rules: a descendant form would match a nested shell's bar
(SPEC-R1b's depth-2 recursion), the exact trap `_page.css:84-93` documents having been measured live
in review.
...
the three carded parts are re-positioned as **floating overlays** by six separate rule-arms
(`:333-343` mid-window, `:424-438` narrow, `:531-541` compact), where the card is `position:absolute`
...
re-creates the original page-background wedge at the header on every narrow-stack shell — the docs
shell's own default at 360px).
...
`margin-block-start: calc(var(--ui-super-shell-module) / 3)`, the same module fraction its existing
`padding-block-end` already uses (`:233`), keeping the strip's two seams symmetric and both on
R11a's ladder.
...
24, 32}). The strip's `calc(var(--ui-super-shell-module) / 3)` has no literal arm. **Therefore the
AC19 leg's negative control must NOT be anchored on `1px`** — that literal also occurs in
...
- **M3 — flush with no separator at all.** The minimal reading of "edge-to-edge bars sit flush".
  ...
  into the rail/pane columns as one L-shaped blob. Note that `super-shell.html`'s demo bars are
  TINTED (`site/pages/super-shell.css:125-126`, `primary-container`), which **masks** this defect on
  the one page a reviewer is most likely to open — a reason to disbelieve that page as evidence, not
  a reason to ship M3.
