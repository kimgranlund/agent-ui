...
> | **Ratified by** | *(pending — Kim; route through an independent `doc-reviewer` pass first: the author of this supersession must not grade it. On acceptance the ratifier flips ADR-0086 to `superseded` — the flip is the ratifier's act, not the author's.)* |
