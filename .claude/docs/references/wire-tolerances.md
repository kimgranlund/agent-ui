...
| S1 | A non-string or unrecognized `catalogId` on a produce request degrades to the default catalog fallback (never a 400/500, never a mixed catalog+prompt) | `packages/agent-ui/a2ui/src/renderer/wire-tolerances.ts:124-129` (`selectCatalog`; `tools/agent/chat-validation.ts` now re-exports it UNCHANGED for its two live-transport callers) | ADR-0169 cl.3 (server-side catalog selection, "fail-closed" ruling) | `packages/agent-ui/a2ui/src/live-agent/select-catalog.test.ts` (GH #487 — fallback/known-id/malformed-id arms, closing the gap this row previously flagged) | Silent — there is no regression trip-wire today if `selectCatalog`'s fail-closed default were accidentally inverted (e.g. throwing, or falling through to `undefined`). |
...
- ~~S1 (`selectCatalog`) has no test anywhere in the repo that pins its fail-closed behavior by
  name.~~ **Closed**: `packages/agent-ui/a2ui/src/live-agent/select-catalog.test.ts` pins the
