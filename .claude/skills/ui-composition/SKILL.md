...
description: >-
  ...
  to this flow". NOT for whole-screen structure/scroll/shell (layout-composition), an
  application's packages/routing/theming spine (app-composition), authoring an A2UI
  payload (a2ui-payload-authoring), or building a new control (component-build).
user-invocable: true
...
1. **Pick the controls** — [[component-catalog]]: the choosing guide's by-job groups, then
   ...
   [[component-design]], never a hand-rolled substitute on a shared surface.
2. **Structure the fragment** — slots and `data-role` roles per the descriptor; content in
   ...
   model ([[composition-patterns]]) — not ad-hoc margins inside a boxed region.
...
7. **Prove it** — `npm run check` type-clean; a behavior probe for the fragment's own logic
   ...
   claim — [[component-testing]]'s jsdom blind-spot list says which); honest states
