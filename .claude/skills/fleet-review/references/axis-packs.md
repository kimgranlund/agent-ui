# Axis packs — canon + ratified-deviations ledger, per axis

Loaded by a reviewer working ONE axis (Phase 2 — canon pack, in `SKILL.md`). Each pack names the
slice, the canon docs, the gold exemplar(s), the checklist, and the ratified-deviations ledger a
reviewer must not re-flag as a finding.
...
- **Canon:** `dom/element.ts`/`dom/form.ts` contract comments · `interaction-states.md` §3 ·
  [[component-patterns]] rows. **Gold:** `controls/checkbox/checkbox.ts` (leaf) ·
