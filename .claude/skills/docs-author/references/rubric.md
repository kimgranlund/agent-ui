...
| D5 | Scaffold-only | [gate] | The page owns only layout chrome and **never restyles a `ui-*` control**; it imports `_page.ts` first (the foundation cascade); slot/role names are canonical | 1: page CSS sets `:hover`/colour on a control, or `_page.ts` isn't first · 3: page CSS is scaffold-only (consumes `--md-sys-color-*` roles); `_page.ts` first; `site-canon` green · 5: + appearance/states are demonstrably the control's, the page adds no `tabindex`/ARIA the control owns |
| D6 | Voice & retrievability | [review] | Precise, honest, scannable prose that *shows* over asserts, **cites** canonical docs by ID rather than restating them, and keeps **provenance** citations (which TKT/ADR built this) out of descriptive prose — in a page-end Changelog table instead (`best-practices.md`'s provenance-vs-normative split) | 1: marketing claims the render can't back, restated `docs/` prose, or a `TKT-####`/build-receipt ADR woven into the intro or a heading · 3: plain declarative, honest labels, cites rule-owners inline, provenance citations (if any) live only in the page-end Changelog · 5: + every claim is reproducible by manipulating the live page; zero restated canon; zero provenance citations anywhere in prose or headings |
