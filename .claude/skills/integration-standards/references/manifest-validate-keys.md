...
(accepted 2026-08-04) and SPEC `.claude/docs/spec/a2ui-live-agent.spec.md` §3.6 (SPEC-R16–R18).
This file states the laws; the cited requirement ids/symbols are the authority — Grep them in the
...
section-number anchors (`SPEC-R16 §3.6`, etc.), not absolute line numbers, precisely because the
spec's line count grows across versions (verified stable v0.13→v0.17) — an anchor never goes stale
on a spec bump, so there is nothing to repair here on drift.
...
*(ADR-0168 cl.1 :52-58, cl.2 :60-65 · SPEC-R16 §3.6)*
...
only, never the first line. *(ADR-0168 cl.3 :67-75 · SPEC-R17 §3.6)*
...
by SPEC-N2. *(ADR-0168 cl.4 :77-84 · SPEC-R18 §3.6)*
