...
| D4 | Non-obvious, frame-led | [review] | Adds project-specific/non-obvious content, no general-knowledge lecturing; each instruction **leads with its reasoning-frame word** (imperative/priority/conditional) over connective filler | 1: lectures the model on its own domain, or opens lines with filler (`It is important to…`) · 3: some filler; frames often buried mid-sentence · 5: every line passes the calibration test — operative word front-loaded, reasoning-implied, one frame per line |
