...
# project-facts — agent-ui's decision and work record
...
spine) is authoritative from `.claude/skills/doc-standards/SKILL.md` — consult it
...
2. Answer with **the claim + the file path or issue URL (+ the record's status where it has
   ...
   for SPEC/LLD/PRD, Issue state + labels on GitHub) — see `doc-standards` before
