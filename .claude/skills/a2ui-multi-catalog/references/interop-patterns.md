# Multi-catalog interop — the ADR-0169 clauses, worked

Harvested verbatim from `a2ui-multi-catalog`'s own body (2026-08-15 extraction, GH #931/wave-2
W2-7). The SKILL.md's five-line index + routing boundary stay inline; this file is the worked
explanation behind each of the five patterns — read it once the index has told you which
section applies. `cl.N` citations resolve against
`.claude/docs/adr/0169-a2ui-basic-catalog-upstream-interop.md`'s Decision clauses (see the
Citation key in SKILL.md for the full convention).
