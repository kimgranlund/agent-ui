...
name: authoring-docs
description: >-
  Author or maintain a page of the agent-ui docs SITE (`site/`) to the standard shape — pick the
  content type, derive the page from its canonical source (the {name}.md descriptor, the real renderer,
  the props enum, the token roles) instead of hand-maintaining it, wire the deterministic drift-gate that
  backs it, and score it against the type's rubric. Use when adding or updating any `/site` page — a
  landing, the permutation matrix, the states showcase, a component API doc, the live A2UI demo, a
  conceptual guide, a family overview, a playground, or a recipe — or auditing the site for drift
  ("document ui-text-field", "add an API page", "is this doc going to drift", "write the theming guide").
---
...
[[authoring-components]], it is **anti-drift by construction**: the right page derives from a canonical
...
- **Use** when adding or changing any `/site` page, documenting a new component, or auditing the site
  ...
  "will this doc drift".
- **Skip** for the *referential* docs under `docs/` (plan / goals / process / `references/`) — those are
  authored with [[authoring-references]], not this. This skill owns the **published site**, not the
  repo's internal knowledge docs.
- **Generator, not critic.** You author *to* the rubric; a **separate** reviewer (the
  `docs-site-steward` agent, or the host gate) scores the page against it. Don't grade your own page —
  build it to clear the gate, then hand off.

## The cardinal discipline — derive, never hand-maintain

A docs page restates nothing it can **derive**. Every fact on a page has a single owner elsewhere in the
repo; the page is its *second consumer*, never a hand-transcribed copy (the copy is the precondition for
drift — `docs/process.md`'s drift disease). The four owners a site page derives from:
...
5. **Score** against `references/rubric.md` (cross-cutting dims + the per-type addendum). Build until
   every gate dimension clears; then hand off to the reviewer — you don't score your own page.
...
| T3 | **states showcase** | the live control staged in each interaction state, **honestly labelled** (`states.ts`) | the control's own `{name}.css` (the page never restyles it) | the page sets no state styling; a real activation log proves pointer + keyboard |
...
| T6 | **conceptual guide** | getting-started / theming / architecture prose — the *why* and *how* | the canonical `docs/` it summarizes (cite by ID, never restate) | soft (prose staleness → reviewer judgment); code samples must type-check |
...
- **Soft staleness** — prose drift (T6 guides going stale against `docs/`) isn't mechanically
...
- [ ] Every derivable fact is derived (descriptor / real renderer / enum / roles) — zero hand-maintained
      copies; the page never restyles a `ui-*` control.
...
| `references/best-practices.md` | The cross-cutting content strategy — voice/tone, the do/don't, honest labels |
