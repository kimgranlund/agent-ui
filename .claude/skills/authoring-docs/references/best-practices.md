...
> The cross-cutting how-to depth behind the `authoring-docs` method: the voice, the do/don't, and the
...
- **Soft (prose, ergonomics, "is this idiomatic") → cite + route to review.** Name the upstream owner by
  ID so a reviewer can re-derive, and leave the judgment to the `docs-site-steward` reviewer. Don't
