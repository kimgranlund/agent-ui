...
> on. Distilled 2026-06-28 from `site/`, `docs/process.md`, and the descriptor trip-wires
...
`docs/` (the repo's internal plan / goals / process / `references/`, authored with `authoring-references`).
...
- **Drift** (`docs/process.md`'s disease): the doc and the thing it documents diverge. The cure is
...
reviewer is the `docs-site-steward` agent (or the host gate). This mirrors the `authoring-components` →
