...
copies — and never from inference. It builds; it never grades its own output (the `a2ui-reviewer`
...
(GH #764/D3 extracted the method out of this body — mirror `a2ui-composer` → `a2ui-compose`).
...
rule 2). Component-side changes (a new prop/event/two-way bind) are `component-builder` territory:
