import"./super-shell-D76CLu9A.js";import{n as e}from"./_page-DIBN49D1.js";import{t}from"./code-block-DEt2Scp8.js";import{a as n,c as r,g as i,h as a,m as o,n as s}from"./doc-page-H_CmxYv1.js";import{t as c}from"./frontmatter-D6AIzGjv.js";/* empty css                   */import{a as l,i as u}from"./specimens-BSFejhGR.js";import{t as d}from"./package-BjQHOzmo.js";var f=`// element.ts — \`UIElement\`, the FACE custom-element host (plan §5).
//
...
`;function p(e,t){let n=e.indexOf(t);if(n===-1)throw Error(`events.ts: method "${t}" not found in dom/element.ts — renamed or removed?`);let r=e.indexOf(`{`,n),i=0,a=r;for(;a<e.length;a++)if(e[a]===`{`)i++;else if(e[a]===`}`&&(i--,i===0)){a++;break}return e.slice(n,a)}function m(...e){let t=document.createElement(`p`);for(let n of e)t.append(typeof n==`string`?document.createTextNode(n):n);return t}function h(e){let t=document.createElement(`code`);return t.textContent=e,t}var{content:g}=e({title:`Events`,intro:"The closed event vocabulary every `ui-*` control emits from — seven names, no more (CLAUDE.md’s Conventions section; naming.md §4). This page derives the mechanics from the real `UIElement.emit` seam, the per-control inventory from every shipped descriptor’s `events[]` sequence, and cites the ADR that minted each name — plus a live event log over a real ui-select and ui-switch."});g.append(n(2,`The closed seven — provenance`)),g.append(m(`A new event name is an ADR-level decision, never a drive-by addition (naming.md §4, `,h(`.claude/docs/references/naming.md:95`),`). Hand-authored — no machine-readable "which ADR minted this name" index exists to parse (the same underivable-provenance posture `,h(`doc-page.ts`),`’s own `,h(`renderChangelogTable`),` comment states); re-verify each row against the cited record.`));{let e=document.createElement(`table`);e.append(o(`Event`,`Minted by`,`Note`));let t=document.createElement(`tbody`);for(let[e,n,r]of[[`change · input · select · open · close · toggle`,`plan.md §9`,'The original six (plan.md:292 — "simple names only"), mechanized as a gate by ADR-0081 (2026-07-05), which also carves out native `click` for a pure-activation control (Amendment 2).'],[`toggle — Shape 1 (POST-commit lifecycle)`,`ADR-0101`,`2026-07-08 — "overlay open-state transitions always announce": the trait becomes the sole announcer, firing AFTER the host’s own state write settles.`],[`toggle — Shape 2 (PRE-commit, cancelable)`,`ADR-0179 (GH #686 Amendment S7-a)`,"2026-08-09 — ui-toggle mint (a pressed-state pill button) reuses `toggle`, PRE-commit via the base `emit()` seam’s `cancelable:true` default, rather than minting an eighth name."],[`action`,`ADR-0153 (GH #147)`,`2026-07-20 — ui-status-stream’s inline retry affordance: the seventh member, "a user committed a per-entry action" (naming.md §4:92).`]])t.append(a(s(e),i(n),i(r)));e.append(t),g.append(e)}g.append(n(2,`UIElement.emit — the mechanics`)),g.append(m(`Every event a control emits rides this ONE seam (`,h(`packages/agent-ui/components/src/dom/element.ts:262`),`) — sliced verbatim, never retyped:`)),g.append(t(p(f,`emit<D = undefined>(type: string, detail?: D): boolean {`),`ts`)),g.append(m(`Every emitted event is `,h(`bubbles: true`),`, `,h(`composed: true`),` (crosses a shadow boundary, though the fleet is light-DOM by default), and `,h(`cancelable: true`),` — a listener’s `,h(`preventDefault()`),` returns `,h(`false`),` from `,h(`emit()`),`, which a control MAY honour (`,h(`ui-toggle`),"’s Shape-2 pre-commit `toggle` is the one shipped consumer that does, below).")),g.append(n(2,`Commit semantics`)),g.append(m(`Paraphrased from naming.md §4 (`,h(`.claude/docs/references/naming.md:96`),`–101) — cite the source, don’t trust this restatement:`));{let e=document.createElement(`table`);e.append(o(`Event`,`Timing`,`Never fires from`));let t=document.createElement(`tbody`);for(let[e,n,r]of[[`change`,`A user commit`,`A programmatic property write`],[`select`,`A user commit (a list-item/option selection)`,`A programmatic property write`],[`input`,`Live, on every interim edit`,`—`],[`open · close · toggle`,`Overlay/disclosure lifecycle, announced by the transition itself`,`—`],[`action`,`A user-committed per-entry action click`,`A programmatic update()/appendEntry() write`]])t.append(a(s(e),i(n),i(r)));e.append(t),g.append(e)}g.append(n(2,`toggle — two shapes, one name`)),g.append(m(`A vocabulary member is closed on NAME, not on timing/cancelability (naming.md §4:103–118). `,_(`Shape 1`),` (the original, overlay/disclosure lifecycle) fires `,_(`after`),` the host’s own state write has settled — `,h(`traits/overlay.ts`),` is the sole announcer (ADR-0101). `,_(`Shape 2`),` (`,h(`ui-toggle`),`, ADR-0179 GH #686 Amendment S7-a) fires `,_(`before`),` the host’s own `,h(`pressed`),` write, riding the base `,h(`emit()`),` seam’s `,h(`cancelable: true`),` default — a listener’s `,h(`preventDefault()`),` refuses the press. A consumer distinguishes the two only by which control it listens to, never by inspecting the event itself.`));function _(e){let t=document.createElement(`strong`);return t.textContent=e,t}g.append(n(2,`Per-control event inventory`)),g.append(m(`Every shipped control (`,h(`components/src/controls/*/*.md`),`) whose descriptor declares at least one event, rendered through `,h(`renderEventsTable`),` — the SAME table a control’s own API doc page renders. Router/code/app-tier descriptors sit outside this components-scoped glob (the same boundary `,h(`site-coverage.test.ts`),` holds) — not covered here.`));{let e=c();if(e.length===0)throw Error(`events.ts: 0 controls with events[] found — descriptor glob broken?`);for(let t of e){let e=r(t.doc.descriptor);e&&(g.append(n(3,t.tag)),g.append(e))}}g.append(n(2,`@agent-ui/components/descriptor`)),g.append(m(`The subpath (`,h(JSON.parse(d).exports[`./descriptor`]),", from the real package.json exports map) barrels the canonical `{name}.md` frontmatter reader (",h(`parseDescriptor`),` / `,h(`splitFrontmatter`),") and its `ParsedDescriptor` schema (ADR-0004). Every control doc page — and this page’s own per-control event inventory above — is a SECOND consumer of that ONE parser, the same one the in-package contract trip-wire (",h(`component-descriptor-driftwire.test.ts`),`) validates against: one parser, many consumers, never a forked frontmatter dialect (`,h(`site/lib/frontmatter.ts`),`, the same adapter every `,h(`*-doc.ts`),` page uses).`)),g.append(n(2,`Live — ui-select + ui-switch`)),g.append(m(`Two real controls, one shared log — every emitted event, in commit order:`));var v=document.createElement(`ul`);v.className=`event-log`,v.setAttribute(`aria-live`,`polite`);var y=0;function b(e){y+=1;let t=document.createElement(`li`);t.textContent=`#${String(y).padStart(2,`0`)}  ${e}`,v.append(t),v.scrollTop=v.scrollHeight}var x=u(`ui-select`,{name:`plan`,placeholder:`Choose a plan…`},[u(`div`,{role:`option`,value:`free`},[document.createTextNode(`Free`)]),u(`div`,{role:`option`,value:`pro`},[document.createTextNode(`Pro`)])]);x.addEventListener(`select`,e=>b(`ui-select   select  detail=${JSON.stringify(e.detail)}`)),x.addEventListener(`toggle`,()=>b(`ui-select   toggle  (open-state transition)`)),x.addEventListener(`close`,()=>b(`ui-select   close`));var S=u(`ui-switch`,{name:`notifications`});S.addEventListener(`change`,()=>b(`ui-switch   change  checked=${S.checked}`)),S.addEventListener(`input`,()=>b(`ui-switch   input   checked=${S.checked}`));var C=u(`div`,{},[x,S]);C.style.cssText=`display:flex; gap:1.5rem; align-items:center; flex-wrap:wrap;`,g.append(l(`Live ui-select + ui-switch`,C),l(`Event log`,v));