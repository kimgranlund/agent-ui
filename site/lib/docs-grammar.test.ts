...
describe('STRUCTURAL — S5-S9 (S5-S8 promoted at Phase 5, manifest M7-M10; S9 minted 2026-07-30)', () => {
  ...
  it('S8: ADR numbering contiguous modulo the KNOWN_GAPS allowlist', () => {
    ...
    // 188: reserved by PR #852's ADR-0188 (ui-drawer component-design intake, GH #845, in-flight
    ...
    // 209/210/211: reserved by in-flight open PRs #1357 (ADR-0209, Disclosure summary slot),
    // #1360 (ADR-0210, FileDrop), #1364 (ADR-0211, Drill catalog mint) — this branch's ADR-0212
    // (GH #1365) was host-assigned the next free number past them; gaps inert once those PRs land.
    const KNOWN_GAPS = new Set([108, 151, 155, 173, 183, 188, 192, 193, 209, 210, 211]) // 192/193 reserved in flight (@agent-ui/data planner · GH #959) when 0194 was minted (GH #975 renumber, 0191 taken by #974) — inert once they land
