...
// mechanism rationale). `componentKnobs()` grows a SLOT_TEXT knob for EVERY component-mode target; applying it
// (`el.textContent =`) is safe for a genuine text/label slot (SLOT_TEXT_OK) but DESTROYS a control's own
// structural children (NO_SLOT_TEXT). Both sets are hand-maintained (an explicit per-tag partition, mirroring
// COMPONENT_SAMPLE_CHILDREN / a2ui-mode's A2UI_INITIAL — Kim's ruling: NOT a runtime children-count heuristic,
// which ui-text's self-healing stamp/observer falsifies). This gate is the PIN: it asserts the two sets
// PARTITION the live fleet exactly, so a new control landing in NEITHER (the silent-drift failure mode a
// runtime check can't catch either) fails the build loud, rather than quietly inheriting SLOT_TEXT_OK by
// default and shipping a specimen that clobbers its own structure — the exact defect this file closes.
...
/** Symmetric partition diff: `missing` (a fleet tag in NEITHER set) + `overlap` (a tag in BOTH — ambiguous). */
function partitionDiff(
  ...
  return {
    missing: [...fleet].filter((t) => !a.has(t) && !b.has(t)).sort(),
    overlap: [...a].filter((t) => b.has(t)).sort(),
  }
}

describe('component-preview SLOT_TEXT partition — anti-vacuous', () => {
  it('found real fleet tags, and both hand-maintained sets are non-empty', () => {
...
describe('component-preview SLOT_TEXT partition — NO_SLOT_TEXT + SLOT_TEXT_OK cover the fleet exactly, no overlap', () => {
  const FLEET = fleetTags()

  it('every NO_SLOT_TEXT / SLOT_TEXT_OK entry is a REAL shipped tag (no stale name)', () => {
  ...
  it('every fleet tag is classified into EXACTLY ONE of NO_SLOT_TEXT / SLOT_TEXT_OK', () => {
    expect(partitionDiff(FLEET, NO_SLOT_TEXT, SLOT_TEXT_OK)).toEqual({ missing: [], overlap: [] })
  })
...
describe('component-preview SLOT_TEXT partition — the gate BITES (synthetic negative controls)', () => {
  it('flags a fleet tag classified in NEITHER set (the silent-drift failure mode this gate closes)', () => {
    const fleet = new Set(['ui-a', 'ui-b', 'ui-zzfake'])
    const a = new Set(['ui-a'])
    const b = new Set(['ui-b'])
    expect(partitionDiff(fleet, a, b)).toEqual({ missing: ['ui-zzfake'], overlap: [] })
  })

  it('flags a tag classified in BOTH sets (an ambiguous, contradictory classification)', () => {
    const fleet = new Set(['ui-a', 'ui-b'])
    const a = new Set(['ui-a', 'ui-b'])
    const b = new Set(['ui-b'])
    expect(partitionDiff(fleet, a, b)).toEqual({ missing: [], overlap: ['ui-b'] })
  })

  it('a clean, exact partition reports empty both ways', () => {
    const fleet = new Set(['ui-a', 'ui-b'])
    expect(partitionDiff(fleet, new Set(['ui-a']), new Set(['ui-b']))).toEqual({ missing: [], overlap: [] })
  })
})
